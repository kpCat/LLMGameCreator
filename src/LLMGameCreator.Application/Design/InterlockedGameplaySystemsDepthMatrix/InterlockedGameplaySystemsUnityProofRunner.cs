...
public sealed class InterlockedGameplaySystemsUnityProofRunner
{
    public static InterlockedUnityProof NotRequested(InterlockedUnityCommandPlan commandPlan) =>
        Blocked(
            "goal065.unity.not_requested",
            "Unity execution was not requested for this validation pass.",
            Summary(
                [InterlockedGameplayDiagnostic.Warning("goal065.unity.not_requested", "unity_cli", "Unity build/player proof was not requested.")],
                unityEditorExecuted: false,
                playerExecuted: false,
                commandPlan: commandPlan));

    public InterlockedUnityProof Run(
        ...
        InterlockedGameplaySystemsOptions options)
    {
        ...
        {
            return Blocked(
                "goal065.unity.project_missing",
                "Repository-local Unity Alpha project or build entrypoint was not found.",
                Summary([Error("goal065.unity.project_missing", "unity/LLMGameCreatorAlpha", "Unity Alpha project and build entrypoint are required.")], false, false, commandPlan: commandPlan));
        }
        ...
        {
            return Blocked(
                "goal065.unity.unity_not_found",
                "Unity Editor executable was not found in PATH or standard Unity Hub locations.",
                Summary([Error("goal065.unity.unity_not_found", "unity_cli", "Unity Editor executable was not found.")], false, false, commandPlan: commandPlan));
        }
        ...
        diagnostics.Add(Info("goal065.unity.editor_executed", "logs/unity-build.log", "Unity Editor was invoked through the existing Alpha build entrypoint."));
        diagnostics.Add(unityResult.ExitCode == 0
            ? Info("goal065.unity.editor_exit_success", "exit_code:0", "Unity Editor build process exited successfully.")
            : Error("goal065.unity.editor_exit_failure", "exit_code:" + unityResult.ExitCode, "Unity Editor build process did not exit successfully."));

        if (unityResult.ExitCode != 0)
        {
            Cleanup(workRoot, options);
            return Blocked(
                "goal065.unity.editor_exit_failure",
        ...
        {
            diagnostics.Add(Error("goal065.unity.executable_missing", "build/b/LLMGameCreatorAlpha.exe", "Unity build did not produce the Alpha executable."));
            Cleanup(workRoot, options);
            return Blocked(
                "goal065.unity.executable_missing",
        ...
        diagnostics.Add(Info("goal065.unity.player_executed", "logs/alpha-player-play-loop.log", "The produced Alpha player was launched in interlocked-gameplay marker mode."));
        diagnostics.Add(playerResult.ExitCode == 0
            ? Info("goal065.unity.player_exit_success", "exit_code:0", "Alpha player process exited successfully.")
            : Error("goal065.unity.player_exit_failure", "exit_code:" + playerResult.ExitCode, "Alpha player process did not exit successfully."));
        ...
        return new InterlockedUnityProof
        {
            Passed = passed,
            UnityEditorOrPlayerExecuted = true,
            BlockerCode = passed ? string.Empty : "goal065.unity.marker_or_player_failure",
            BlockerMessage = passed ? string.Empty : "Unity player did not produce all required Goal 065 interlocked gameplay markers or exited with failure.",
            PlayerProof = summary with { Passed = passed },
            Diagnostics = SortDiagnostics(diagnostics.Concat(summary.Diagnostics))
        };
    }

    private static InterlockedUnityProof Blocked(
        string blockerCode,
        string blockerMessage,
        InterlockedUnityProofSummary summary) =>
        new()
        {
            Passed = false,
    ...
    private static InterlockedUnityProofSummary Summary(
        IEnumerable<InterlockedGameplayDiagnostic> diagnostics,
        ...
        string launchLog = "",
        string playLoopLog = "",
        InterlockedUnityCommandPlan? commandPlan = null)
    {
        ...
        foreach (var marker in missing)
        {
            summaryDiagnostics.Add(playerExecuted
                ? Error("goal065.unity.marker_missing", marker, "Unity player logs did not contain the required Goal 065 marker.")
                : Warning("goal065.unity.marker_not_checked", marker, "Unity player was not executed, so the Goal 065 marker could not be checked."));
        }

        return new InterlockedUnityProofSummary
        {
            ...
            ProvenRowCount = commandPlan?.Rows.Count(row => lines.Contains("interlocked_gameplay_row_completed=" + row.RowId, StringComparer.Ordinal)) ?? 0,
    ...
    private static void Cleanup(string workRoot, InterlockedGameplaySystemsOptions options)
    {
    ...
    private static string RelativeGoalPath(string path)
    {
        ...
        var marker = "/" + InterlockedGameplaySystemsDepthMatrixVocabulary.RelativeOutputDirectory.TrimStart('/').Replace('\\', '/') + "/";
        var index = normalized.IndexOf(marker, StringComparison.OrdinalIgnoreCase);
        return index < 0
            ? string.Empty
            : InterlockedGameplaySystemsDepthMatrixVocabulary.RelativeOutputDirectory + "/" + normalized[(index + marker.Length)..];
    }

    private static IReadOnlyList<InterlockedGameplayDiagnostic> SortDiagnostics(IEnumerable<InterlockedGameplayDiagnostic> diagnostics) =>
        InterlockedGameplaySystemsValidator.Sort(diagnostics);
    ...
    private static InterlockedGameplayDiagnostic Warning(string code, string target, string message) =>
        InterlockedGameplayDiagnostic.Warning(code, target, message);

    private static InterlockedGameplayDiagnostic Info(string code, string target, string message) =>
        InterlockedGameplayDiagnostic.Info(code, target, message);
