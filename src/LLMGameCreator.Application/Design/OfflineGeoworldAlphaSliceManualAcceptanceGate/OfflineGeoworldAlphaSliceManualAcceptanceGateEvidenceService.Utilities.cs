...
public sealed partial class OfflineGeoworldAlphaSliceManualAcceptanceGateEvidenceService
{
    ...
    private static void ResetCurrentGoalDirectory(string root, string path)
    {
        ...
        if (!IsSubPath(root, path)
            || !normalized.Contains("goal-110", StringComparison.OrdinalIgnoreCase)
            && !normalized.Contains("OfflineGeoworldGoal110", StringComparison.Ordinal))
        {
            throw new InvalidOperationException("Refusing to reset a non-Goal110 directory: " + path);
        }
    ...
    private static IReadOnlyDictionary<string, string> SnapshotHashes(
        string root,
    ...
    private static readonly HashSet<string> ForbiddenBinaryOrRasterExtensions = new(
