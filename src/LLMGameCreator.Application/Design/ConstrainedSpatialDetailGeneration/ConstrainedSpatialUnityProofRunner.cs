...
public sealed class ConstrainedSpatialUnityProofRunner
{
    public ConstrainedSpatialUnityProof Run(
        string projectRootPath,
        string outputDirectoryPath,
        string stagingDirectoryPath,
        ConstrainedSpatialUnityCommandPlan commandPlan,
        ConstrainedSpatialDetailOptions options)
    {
        ...
        {
            return Blocked(
                "goal062.unity.not_requested",
                "Unity execution was not requested for this validation pass.",
                Summary([Warning("goal062.unity.not_requested", "unity_cli", "Unity build/player proof was not requested.")], false, false));
        }
        ...
        {
            return Blocked(
                "goal062.unity.project_missing",
                "Repository-local Unity Alpha project or build entrypoint was not found.",
                Summary([Error("goal062.unity.project_missing", "unity/LLMGameCreatorAlpha", "Unity Alpha project and build entrypoint are required.")], false, false));
        }
        ...
        {
            return Blocked(
                "goal062.unity.unity_not_found",
                "Unity Editor executable was not found in PATH or standard Unity Hub locations.",
                Summary([Error("goal062.unity.unity_not_found", "unity_cli", "Unity Editor executable was not found.")], false, false));
        }
        ...
        diagnostics.Add(Info("goal062.unity.editor_executed", "logs/unity-build.log", "Unity Editor was invoked through the existing Alpha build entrypoint."));
        diagnostics.Add(unityResult.ExitCode == 0
            ...
            : Error("goal062.unity.editor_exit_failure", "exit_code:" + unityResult.ExitCode, "Unity Editor build process did not exit successfully."));

        if (unityResult.ExitCode != 0)
        {
            Cleanup(workRoot, options);
            return Blocked(
                "goal062.unity.editor_exit_failure",
        ...
        {
            diagnostics.Add(Error("goal062.unity.executable_missing", "build/b/LLMGameCreatorAlpha.exe", "Unity build did not produce the Alpha executable."));
            Cleanup(workRoot, options);
            return Blocked(
                "goal062.unity.executable_missing",
        ...
        diagnostics.Add(Info("goal062.unity.player_executed", "logs/alpha-player-play-loop.log", "The produced Alpha player was launched in spatial-detail play-loop mode."));
        diagnostics.Add(playerResult.ExitCode == 0
            ...
            : Error("goal062.unity.player_exit_failure", "exit_code:" + playerResult.ExitCode, "Alpha player process did not exit successfully."));
        ...
        return new ConstrainedSpatialUnityProof
        {
            Passed = passed,
            UnityEditorOrPlayerExecuted = true,
            BlockerCode = passed ? string.Empty : "goal062.unity.marker_or_player_failure",
            BlockerMessage = passed ? string.Empty : "Unity player did not produce all required Goal 062 spatial-detail markers or exited with failure.",
            PlayerProof = summary with { Passed = passed },
            Diagnostics = SortDiagnostics(diagnostics.Concat(summary.Diagnostics))
        };
    }

    private static ConstrainedSpatialUnityProof Blocked(
        string blockerCode,
        string blockerMessage,
        ConstrainedSpatialUnityProofSummary summary) =>
        new()
        {
            Passed = false,
    ...
    private static ConstrainedSpatialUnityProofSummary Summary(
        IEnumerable<ConstrainedSpatialDiagnostic> diagnostics,
        ...
        string launchLog = "",
        string playLoopLog = "",
        ConstrainedSpatialUnityCommandPlan? commandPlan = null)
    {
        ...
        foreach (var marker in missing)
        {
            summaryDiagnostics.Add(playerExecuted
                ? Error("goal062.unity.marker_missing", marker, "Unity player logs did not contain the required Goal 062 spatial-detail marker.")
                : Warning("goal062.unity.marker_not_checked", marker, "Unity player was not executed, so the Goal 062 spatial-detail marker could not be checked."));
        }

        return new ConstrainedSpatialUnityProofSummary
        {
            ...
            ProvenRowCount = commandPlan?.Rows.Count(row => lines.Contains("spatial_detail_row=" + row.RowId, StringComparer.Ordinal)) ?? 0,
    ...
    private static void Cleanup(string workRoot, ConstrainedSpatialDetailOptions options)
    {
    ...
    private static string RelativeGoalPath(string path)
    {
        ...
        var marker = "/" + ConstrainedSpatialDetailVocabulary.RelativeOutputDirectory.TrimStart('/').Replace('\\', '/') + "/";
        var index = normalized.IndexOf(marker, StringComparison.OrdinalIgnoreCase);
        return index < 0
            ? string.Empty
            : ConstrainedSpatialDetailVocabulary.RelativeOutputDirectory + "/" + normalized[(index + marker.Length)..];
    }

    private static IReadOnlyList<ConstrainedSpatialDiagnostic> SortDiagnostics(IEnumerable<ConstrainedSpatialDiagnostic> diagnostics) =>
        ConstrainedSpatialDetailValidator.Sort(diagnostics);
    ...
    private static ConstrainedSpatialDiagnostic Warning(string code, string target, string message) =>
        ConstrainedSpatialDiagnostic.Warning(code, target, message);
