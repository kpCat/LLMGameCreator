...
public sealed class FullCampaignPlayableReviewPackageRcUnityProofRunner
{
    public FullCampaignPlayableUnityProof Run(
        string projectRootPath,
        string outputDirectoryPath,
        string stagingDirectoryPath,
        FullCampaignPlayableUnityCommandPlan commandPlan,
        FullCampaignPlayableReviewPackageRcOptions options)
    {
        ...
        {
            return Blocked(
                "goal061.unity.not_requested",
                "Unity execution was not requested for this validation pass.",
                Summary(
                    [Warning("goal061.unity.not_requested", "unity_cli", "Unity build/player proof was not requested.")],
        ...
        {
            return Blocked(
                "goal061.unity.project_missing",
                "Repository-local Unity Alpha project or build entrypoint was not found.",
                Summary([Error("goal061.unity.project_missing", "unity/LLMGameCreatorAlpha", "Unity Alpha project and build entrypoint are required.")], false, false));
        }
        ...
        {
            return Blocked(
                "goal061.unity.unity_not_found",
                "Unity Editor executable was not found in PATH or standard Unity Hub locations.",
                Summary([Error("goal061.unity.unity_not_found", "unity_cli", "Unity Editor executable was not found.")], false, false));
        }
        ...
        diagnostics.Add(Info("goal061.unity.editor_executed", "logs/unity-build.log", "Unity Editor was invoked through the existing Alpha build entrypoint."));
        diagnostics.Add(unityResult.ExitCode == 0
            ? Info("goal061.unity.editor_exit_success", "exit_code:0", "Unity Editor build process exited successfully.")
            : Error("goal061.unity.editor_exit_failure", "exit_code:" + unityResult.ExitCode, "Unity Editor build process did not exit successfully."));

        if (unityResult.ExitCode != 0)
        {
            Cleanup(workRoot, options);
            return Blocked(
                "goal061.unity.editor_exit_failure",
        ...
        {
            diagnostics.Add(Error("goal061.unity.executable_missing", "build/b/LLMGameCreatorAlpha.exe", "Unity build did not produce the Alpha executable."));
            Cleanup(workRoot, options);
            return Blocked(
                "goal061.unity.executable_missing",
        ...
        diagnostics.Add(Info("goal061.unity.player_executed", "logs/alpha-player-play-loop.log", "The produced Alpha player was launched in review-package RC play-loop mode."));
        diagnostics.Add(playerResult.ExitCode == 0
            ? Info("goal061.unity.player_exit_success", "exit_code:0", "Alpha player process exited successfully.")
            : Error("goal061.unity.player_exit_failure", "exit_code:" + playerResult.ExitCode, "Alpha player process did not exit successfully."));
        ...
        return new FullCampaignPlayableUnityProof
        {
            Passed = passed,
            UnityEditorOrPlayerExecuted = true,
            BlockerCode = passed ? string.Empty : "goal061.unity.marker_or_player_failure",
            BlockerMessage = passed ? string.Empty : "Unity player did not produce all required Goal 061 review-package RC markers or exited with failure.",
            PlayerProof = summary with { Passed = passed },
            Diagnostics = SortDiagnostics(diagnostics.Concat(summary.Diagnostics))
        };
    }

    private static FullCampaignPlayableUnityProof Blocked(
        string blockerCode,
        string blockerMessage,
        FullCampaignPlayableUnityPlayerProof summary) =>
        new()
        {
            Passed = false,
    ...
    private static FullCampaignPlayableUnityPlayerProof Summary(
        IEnumerable<FullCampaignPlayableReviewPackageRcDiagnostic> diagnostics,
        ...
        string launchLog = "",
        string playLoopLog = "",
        FullCampaignPlayableUnityCommandPlan? commandPlan = null)
    {
        ...
        foreach (var marker in missing)
        {
            summaryDiagnostics.Add(playerExecuted
                ? Error("goal061.unity.marker_missing", marker, "Unity player logs did not contain the required Goal 061 review-package RC marker.")
                : Warning("goal061.unity.marker_not_checked", marker, "Unity player was not executed, so the Goal 061 review-package RC marker could not be checked."));
        }

        return new FullCampaignPlayableUnityPlayerProof
        {
            ...
            ProvenRowCount = commandPlan?.Rows.Count(row => lines.Contains("package_row_selected=" + row.RowId, StringComparer.Ordinal)) ?? 0,
    ...
    private static void Cleanup(string workRoot, FullCampaignPlayableReviewPackageRcOptions options)
    {
    ...
    private static string RelativeGoalPath(string path)
    {
        ...
        var marker = "/" + FullCampaignPlayableReviewPackageRcVocabulary.RelativeOutputDirectory.TrimStart('/').Replace('\\', '/') + "/";
        var index = normalized.IndexOf(marker, StringComparison.OrdinalIgnoreCase);
        return index < 0
            ? string.Empty
            : FullCampaignPlayableReviewPackageRcVocabulary.RelativeOutputDirectory + "/" + normalized[(index + marker.Length)..];
    }
    ...
    private static IReadOnlyList<FullCampaignPlayableReviewPackageRcDiagnostic> SortDiagnostics(IEnumerable<FullCampaignPlayableReviewPackageRcDiagnostic> diagnostics) =>
        FullCampaignPlayableReviewPackageRcBuilder.SortDiagnostics(diagnostics);
    ...
    private static FullCampaignPlayableReviewPackageRcDiagnostic Warning(string code, string target, string message) =>
        FullCampaignPlayableReviewPackageRcDiagnostic.Warning(code, target, message);
