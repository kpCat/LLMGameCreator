...
public sealed class UnityAlphaMediaBoundUnityProofRunner
{
    public UnityAlphaMediaBoundLoadProof Run(
        string projectRootPath,
        string outputDirectoryPath,
        string stagingDirectoryPath,
        UnityAlphaMediaBoundLoadContract contract,
        UnityAlphaMediaBoundOptions options)
    {
        ...
        {
            return Blocked(
                "goal056.unity.not_requested",
                "Unity execution was not requested for this validation pass.",
                new UnityAlphaMediaBoundSmokeLogSummary
                {
                    Diagnostics =
                    [
                        Warning("goal056.unity.not_requested", "unity_cli", "Unity build/player proof was not requested.")
                    ]
                });
        }
        ...
        {
            return Blocked(
                "goal056.unity.project_missing",
                "Repository-local Unity Alpha project or build entrypoint was not found.",
                Summary(diagnostics.Concat([Error("goal056.unity.project_missing", "unity/LLMGameCreatorAlpha", "Unity Alpha project and build entrypoint are required.")]), false, false));
        }
        ...
        {
            return Blocked(
                "goal056.unity.unity_not_found",
                "Unity Editor executable was not found in PATH or standard Unity Hub locations.",
                Summary(diagnostics.Concat([Error("goal056.unity.unity_not_found", "unity_cli", "Unity Editor executable was not found.")]), false, false));
        }
        ...
        diagnostics.Add(Info("goal056.unity.editor_executed", "logs/unity-build.log", "Unity Editor was invoked through the existing Alpha build entrypoint."));
        diagnostics.Add(unityResult.ExitCode == 0
            ...
            : Error("goal056.unity.editor_exit_failure", "exit_code:" + unityResult.ExitCode, "Unity Editor build process did not exit successfully."));

        if (!File.Exists(unityBuildLog))
        {
            diagnostics.Add(Error("goal056.unity.build_log_missing", "logs/unity-build.log", "Unity build log was not produced."));
        }
        ...
        if (unityResult.ExitCode != 0)
        {
            ...
            return Blocked(
                "goal056.unity.editor_exit_failure",
                "Unity Editor build process failed; see logs/unity-build.log.",
                Summary(diagnostics, unityEditorExecuted: true, playerExecuted: false, unityResult.ExitCode, null, unityBuildLog, launchLog, playLoopLog, contract));
        }

        var executable = Path.Combine(buildRoot, "LLMGameCreatorAlpha.exe");
        if (!File.Exists(executable))
        {
            diagnostics.Add(Error("goal056.unity.executable_missing", "build/b/LLMGameCreatorAlpha.exe", "Unity build did not produce the Alpha executable."));
            Cleanup(workRoot, options);
            return Blocked(
                "goal056.unity.executable_missing",
        ...
        diagnostics.Add(Info("goal056.unity.player_executed", "logs/alpha-player-play-loop.log", "The produced Alpha player was launched in diagnostic play-loop mode."));
        diagnostics.Add(playerResult.ExitCode == 0
            ...
            : Error("goal056.unity.player_exit_failure", "exit_code:" + playerResult.ExitCode, "Alpha player process did not exit successfully."));

        var summary = Summary(diagnostics, unityEditorExecuted: true, playerExecuted: true, unityResult.ExitCode, playerResult.ExitCode, unityBuildLog, launchLog, playLoopLog, contract);
        var markerSet = summary.MatchedMarkers.ToHashSet(StringComparer.Ordinal);
        var passed = playerResult.ExitCode == 0 && summary.MissingMarkers.Count == 0;
        Cleanup(workRoot, options);

        return new UnityAlphaMediaBoundLoadProof
        {
            Passed = passed,
            UnityEditorOrPlayerExecuted = true,
            ManifestLoadedByUnityProof = markerSet.Contains("media_bound_manifest_loaded=true"),
            PngLoadProofPassed = markerSet.Contains("media_bound_png_loaded=true"),
            WavLoadProofPassed = markerSet.Contains("media_bound_wav_loaded=true"),
            BundleProofPassed = markerSet.Contains("media_bound_bundle_loaded=true"),
            HashValidationPassed = markerSet.Contains("media_bound_hash_validation=true"),
            FamilyMediaPanelProofPassed = UnityAlphaMediaBoundPlayablePackageVocabulary.FamilyIds.All(familyId => markerSet.Contains("media_bound_family_panel_proof=" + familyId)),
            BlockerCode = passed ? string.Empty : "goal056.unity.marker_or_player_failure",
            BlockerMessage = passed ? string.Empty : "Unity player did not produce all required media-bound markers or exited with failure.",
            SmokeLogSummary = summary with { Passed = passed },
            Diagnostics = SortDiagnostics(diagnostics.Concat(summary.Diagnostics))
        };
    }

    private static UnityAlphaMediaBoundLoadProof Blocked(
        string blockerCode,
        string blockerMessage,
        UnityAlphaMediaBoundSmokeLogSummary summary) =>
        new()
        {
            Passed = false,
            ...
            SmokeLogSummary = summary,
            Diagnostics = SortDiagnostics(summary.Diagnostics.Concat([Warning(blockerCode, "unity-proof", blockerMessage)]))
        };

    private static UnityAlphaMediaBoundSmokeLogSummary Summary(
        IEnumerable<UnityAlphaMediaBoundDiagnostic> diagnostics,
        ...
        string launchLog = "",
        string playLoopLog = "",
        UnityAlphaMediaBoundLoadContract? contract = null)
    {
        var required = contract?.RequiredLogMarkers ?? [];
        ...
        foreach (var marker in missing)
        {
            summaryDiagnostics.Add(playerExecuted
                ? Error("goal056.unity.marker_missing", marker, "Unity player logs did not contain the required media-bound marker.")
                : Warning("goal056.unity.marker_not_checked", marker, "Unity player was not executed, so the media-bound marker could not be checked."));
        }

        return new UnityAlphaMediaBoundSmokeLogSummary
        {
    ...
    private static void Cleanup(string workRoot, UnityAlphaMediaBoundOptions options)
    {
    ...
    private static string RelativeGoalPath(string path)
    {
        ...
        var marker = "/" + UnityAlphaMediaBoundPlayablePackageVocabulary.RelativeOutputDirectory.TrimStart('/').Replace('\\', '/') + "/";
        var index = normalized.IndexOf(marker, StringComparison.OrdinalIgnoreCase);
        return index < 0
            ? string.Empty
            : UnityAlphaMediaBoundPlayablePackageVocabulary.RelativeOutputDirectory + "/" + normalized[(index + marker.Length)..];
    }
    ...
    private static UnityAlphaMediaBoundDiagnostic Warning(string code, string target, string message) =>
        UnityAlphaMediaBoundDiagnostic.Warning(code, target, message);
