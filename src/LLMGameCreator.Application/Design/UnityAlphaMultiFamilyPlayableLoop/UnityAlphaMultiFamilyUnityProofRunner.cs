...
public sealed class UnityAlphaMultiFamilyUnityProofRunner
{
    public UnityAlphaMultiFamilyUnityProof Run(
        string projectRootPath,
        string outputDirectoryPath,
        string stagingDirectoryPath,
        UnityAlphaFamilyCommandPlan commandPlan,
        UnityAlphaMultiFamilyOptions options)
    {
        ...
        {
            return Blocked(
                "goal057.unity.not_requested",
                "Unity execution was not requested for this validation pass.",
                new UnityAlphaMultiFamilyPlayerLogSummary
                {
                    Diagnostics =
                    [
                        Warning("goal057.unity.not_requested", "unity_cli", "Unity build/player proof was not requested.")
                    ]
                });
        }
        ...
        {
            return Blocked(
                "goal057.unity.project_missing",
                "Repository-local Unity Alpha project or build entrypoint was not found.",
                Summary(diagnostics.Concat([Error("goal057.unity.project_missing", "unity/LLMGameCreatorAlpha", "Unity Alpha project and build entrypoint are required.")]), false, false));
        }
        ...
        {
            return Blocked(
                "goal057.unity.unity_not_found",
                "Unity Editor executable was not found in PATH or standard Unity Hub locations.",
                Summary(diagnostics.Concat([Error("goal057.unity.unity_not_found", "unity_cli", "Unity Editor executable was not found.")]), false, false));
        }
        ...
        diagnostics.Add(Info("goal057.unity.editor_executed", "logs/unity-build.log", "Unity Editor was invoked through the existing Alpha build entrypoint."));
        diagnostics.Add(unityResult.ExitCode == 0
            ? Info("goal057.unity.editor_exit_success", "exit_code:0", "Unity Editor build process exited successfully.")
            : Error("goal057.unity.editor_exit_failure", "exit_code:" + unityResult.ExitCode, "Unity Editor build process did not exit successfully."));

        if (!File.Exists(unityBuildLog))
        {
            diagnostics.Add(Error("goal057.unity.build_log_missing", "logs/unity-build.log", "Unity build log was not produced."));
        }
        ...
        if (unityResult.ExitCode != 0)
        {
            ...
            return Blocked(
                "goal057.unity.editor_exit_failure",
        ...
        {
            diagnostics.Add(Error("goal057.unity.executable_missing", "build/b/LLMGameCreatorAlpha.exe", "Unity build did not produce the Alpha executable."));
            Cleanup(workRoot, options);
            return Blocked(
                "goal057.unity.executable_missing",
        ...
        diagnostics.Add(Info("goal057.unity.player_executed", "logs/alpha-player-play-loop.log", "The produced Alpha player was launched in multi-family diagnostic play-loop mode."));
        diagnostics.Add(playerResult.ExitCode == 0
            ? Info("goal057.unity.player_exit_success", "exit_code:0", "Alpha player process exited successfully.")
            : Error("goal057.unity.player_exit_failure", "exit_code:" + playerResult.ExitCode, "Alpha player process did not exit successfully."));
        ...
        return new UnityAlphaMultiFamilyUnityProof
        {
            Passed = passed,
            UnityEditorOrPlayerExecuted = true,
            BlockerCode = passed ? string.Empty : "goal057.unity.marker_or_player_failure",
            BlockerMessage = passed ? string.Empty : "Unity player did not produce all required Goal 057 markers or exited with failure.",
            PlayerLogSummary = summary with { Passed = passed },
            Diagnostics = SortDiagnostics(diagnostics.Concat(summary.Diagnostics))
        };
    }

    private static UnityAlphaMultiFamilyUnityProof Blocked(
        string blockerCode,
        string blockerMessage,
        UnityAlphaMultiFamilyPlayerLogSummary summary) =>
        new()
        {
            Passed = false,
            ...
            PlayerLogSummary = summary,
            Diagnostics = SortDiagnostics(summary.Diagnostics.Concat([Warning(blockerCode, "unity-proof", blockerMessage)]))
        };

    private static UnityAlphaMultiFamilyPlayerLogSummary Summary(
        IEnumerable<UnityAlphaMultiFamilyDiagnostic> diagnostics,
        ...
        string launchLog = "",
        string playLoopLog = "",
        UnityAlphaFamilyCommandPlan? commandPlan = null)
    {
        ...
        foreach (var marker in missing)
        {
            summaryDiagnostics.Add(playerExecuted
                ? Error("goal057.unity.marker_missing", marker, "Unity player logs did not contain the required Goal 057 marker.")
                : Warning("goal057.unity.marker_not_checked", marker, "Unity player was not executed, so the Goal 057 marker could not be checked."));
        }

        return new UnityAlphaMultiFamilyPlayerLogSummary
        {
    ...
    private static void Cleanup(string workRoot, UnityAlphaMultiFamilyOptions options)
    {
    ...
    private static string RelativeGoalPath(string path)
    {
        ...
        var marker = "/" + UnityAlphaMultiFamilyPlayableLoopVocabulary.RelativeOutputDirectory.TrimStart('/').Replace('\\', '/') + "/";
        var index = normalized.IndexOf(marker, StringComparison.OrdinalIgnoreCase);
        return index < 0
            ? string.Empty
            : UnityAlphaMultiFamilyPlayableLoopVocabulary.RelativeOutputDirectory + "/" + normalized[(index + marker.Length)..];
    }
    ...
    private static UnityAlphaMultiFamilyDiagnostic Warning(string code, string target, string message) =>
        UnityAlphaMultiFamilyDiagnostic.Warning(code, target, message);
