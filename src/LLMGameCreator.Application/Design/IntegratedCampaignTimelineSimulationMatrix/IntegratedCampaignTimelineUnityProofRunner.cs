...
public sealed class IntegratedCampaignTimelineUnityProofRunner
{
    public static TimelineUnityProof NotRequested(TimelineUnityCommandPlan commandPlan) =>
        Blocked(
            "goal070.unity.not_requested",
            "Unity execution was not requested for this validation pass.",
            Summary(
                [TimelineDiagnostic.Warning("goal070.unity.not_requested", "unity_cli", "Unity build/player proof was not requested.")],
                unityEditorExecuted: false,
                playerExecuted: false,
                commandPlan: commandPlan));

    public TimelineUnityProof Run(
        string projectRootPath,
        string outputDirectoryPath,
        string stagingDirectoryPath,
        TimelineUnityCommandPlan commandPlan,
        IntegratedCampaignTimelineOptions options)
    {
        ...
        {
            return Blocked(
                "goal070.unity.project_missing",
                "Repository-local Unity Alpha project or build entrypoint was not found.",
                Summary([Error("goal070.unity.project_missing", "unity/LLMGameCreatorAlpha", "Unity Alpha project and build entrypoint are required.")], false, false, commandPlan: commandPlan));
        }
        ...
        {
            return Blocked(
                "goal070.unity.unity_not_found",
                "Unity Editor executable was not found in PATH or standard Unity Hub locations.",
                Summary([Error("goal070.unity.unity_not_found", "unity_cli", "Unity Editor executable was not found.")], false, false, commandPlan: commandPlan));
        }
        ...
        diagnostics.Add(Info("goal070.unity.editor_executed", "logs/unity-build.log", "Unity Editor was invoked through the existing Alpha build entrypoint."));
        diagnostics.Add(unityResult.ExitCode == 0
            ...
            : Error("goal070.unity.editor_exit_failure", "exit_code:" + unityResult.ExitCode, "Unity Editor build process did not exit successfully."));

        if (unityResult.ExitCode != 0)
        {
            Cleanup(workRoot, options);
            return Blocked(
                "goal070.unity.editor_exit_failure",
        ...
        {
            diagnostics.Add(Error("goal070.unity.executable_missing", "build/b/LLMGameCreatorAlpha.exe", "Unity build did not produce the Alpha executable."));
            Cleanup(workRoot, options);
            return Blocked(
                "goal070.unity.executable_missing",
        ...
        diagnostics.Add(Info("goal070.unity.player_executed", "logs/alpha-player-play-loop.log", "The produced Alpha player was launched in integrated timeline marker mode."));
        diagnostics.Add(playerResult.ExitCode == 0
            ...
            : Error("goal070.unity.player_exit_failure", "exit_code:" + playerResult.ExitCode, "Alpha player process did not exit successfully."));
        ...
        return new TimelineUnityProof
        {
            Passed = passed,
            UnityEditorOrPlayerExecuted = true,
            BlockerCode = passed ? string.Empty : "goal070.unity.marker_or_player_failure",
            BlockerMessage = passed ? string.Empty : "Unity player did not produce all required Goal 070 timeline markers or exited with failure.",
            PlayerProof = summary with { Passed = passed },
            Diagnostics = SortDiagnostics(diagnostics.Concat(summary.Diagnostics))
        };
    }

    private static TimelineUnityProof Blocked(string blockerCode, string blockerMessage, TimelineUnityProofSummary summary) =>
        new()
        {
            Passed = false,
    ...
    private static TimelineUnityProofSummary Summary(
        IEnumerable<TimelineDiagnostic> diagnostics,
        ...
        string launchLog = "",
        string playLoopLog = "",
        TimelineUnityCommandPlan? commandPlan = null)
    {
        ...
        foreach (var marker in missing)
        {
            summaryDiagnostics.Add(playerExecuted
                ? Error("goal070.unity.marker_missing", marker, "Unity player logs did not contain the required Goal 070 marker.")
                : Warning("goal070.unity.marker_not_checked", marker, "Unity player was not executed, so the Goal 070 marker could not be checked."));
        }

        return new TimelineUnityProofSummary
        {
            ...
            ProvenRowCount = commandPlan?.Rows.Count(row => lines.Contains("campaign_timeline_row_completed=" + row.RowId, StringComparer.Ordinal)) ?? 0,
    ...
    private static void Cleanup(string workRoot, IntegratedCampaignTimelineOptions options)
    {
    ...
    private static string RelativeGoalPath(string path)
    {
        ...
        const string marker = "goal-070-integrated-campaign-timeline-simulation-matrix/";
    ...
    private static IReadOnlyList<TimelineDiagnostic> SortDiagnostics(IEnumerable<TimelineDiagnostic> diagnostics) =>
        diagnostics
    ...
    private static TimelineDiagnostic Warning(string code, string target, string message) =>
        TimelineDiagnostic.Warning(code, target, message);
