...
public sealed class FullGeneratorVariabilityUnityProofRunner
{
    public FullGeneratorVariabilityUnityProof Run(
        string projectRootPath,
        string outputDirectoryPath,
        string stagingDirectoryPath,
        FullGeneratorVariabilityUnityMatrixCommandPlan commandPlan,
        FullGeneratorVariabilityMatrixOptions options)
    {
        ...
        {
            return Blocked(
                "goal059.unity.not_requested",
                "Unity execution was not requested for this validation pass.",
                Summary(
                    [Warning("goal059.unity.not_requested", "unity_cli", "Unity build/player proof was not requested.")],
        ...
        {
            return Blocked(
                "goal059.unity.project_missing",
                "Repository-local Unity Alpha project or build entrypoint was not found.",
                Summary([Error("goal059.unity.project_missing", "unity/LLMGameCreatorAlpha", "Unity Alpha project and build entrypoint are required.")], false, false));
        }
        ...
        {
            return Blocked(
                "goal059.unity.unity_not_found",
                "Unity Editor executable was not found in PATH or standard Unity Hub locations.",
                Summary([Error("goal059.unity.unity_not_found", "unity_cli", "Unity Editor executable was not found.")], false, false));
        }
        ...
        diagnostics.Add(Info("goal059.unity.editor_executed", "logs/unity-build.log", "Unity Editor was invoked through the existing Alpha build entrypoint."));
        diagnostics.Add(unityResult.ExitCode == 0
            ? Info("goal059.unity.editor_exit_success", "exit_code:0", "Unity Editor build process exited successfully.")
            : Error("goal059.unity.editor_exit_failure", "exit_code:" + unityResult.ExitCode, "Unity Editor build process did not exit successfully."));

        if (unityResult.ExitCode != 0)
        {
            Cleanup(workRoot, options);
            return Blocked(
                "goal059.unity.editor_exit_failure",
        ...
        {
            diagnostics.Add(Error("goal059.unity.executable_missing", "build/b/LLMGameCreatorAlpha.exe", "Unity build did not produce the Alpha executable."));
            Cleanup(workRoot, options);
            return Blocked(
                "goal059.unity.executable_missing",
        ...
        diagnostics.Add(Info("goal059.unity.player_executed", "logs/alpha-player-play-loop.log", "The produced Alpha player was launched in matrix diagnostic play-loop mode."));
        diagnostics.Add(playerResult.ExitCode == 0
            ? Info("goal059.unity.player_exit_success", "exit_code:0", "Alpha player process exited successfully.")
            : Error("goal059.unity.player_exit_failure", "exit_code:" + playerResult.ExitCode, "Alpha player process did not exit successfully."));
        ...
        return new FullGeneratorVariabilityUnityProof
        {
            Passed = passed,
            UnityEditorOrPlayerExecuted = true,
            BlockerCode = passed ? string.Empty : "goal059.unity.marker_or_player_failure",
            BlockerMessage = passed ? string.Empty : "Unity player did not produce all required Goal 059 matrix markers or exited with failure.",
            PlayerProof = summary with { Passed = passed },
            Diagnostics = SortDiagnostics(diagnostics.Concat(summary.Diagnostics))
        };
    }

    private static FullGeneratorVariabilityUnityProof Blocked(
        string blockerCode,
        string blockerMessage,
        FullGeneratorVariabilityUnityPlayerProof summary) =>
        new()
        {
            Passed = false,
    ...
    private static FullGeneratorVariabilityUnityPlayerProof Summary(
        IEnumerable<FullGeneratorVariabilityDiagnostic> diagnostics,
        ...
        string launchLog = "",
        string playLoopLog = "",
        FullGeneratorVariabilityUnityMatrixCommandPlan? commandPlan = null)
    {
        ...
        foreach (var marker in missing)
        {
            summaryDiagnostics.Add(playerExecuted
                ? Error("goal059.unity.marker_missing", marker, "Unity player logs did not contain the required Goal 059 marker.")
                : Warning("goal059.unity.marker_not_checked", marker, "Unity player was not executed, so the Goal 059 marker could not be checked."));
        }

        return new FullGeneratorVariabilityUnityPlayerProof
        {
    ...
    private static void Cleanup(string workRoot, FullGeneratorVariabilityMatrixOptions options)
    {
    ...
    private static string RelativeGoalPath(string path)
    {
        ...
        var marker = "/" + FullGeneratorVariabilityMatrixVocabulary.RelativeOutputDirectory.TrimStart('/').Replace('\\', '/') + "/";
        var index = normalized.IndexOf(marker, StringComparison.OrdinalIgnoreCase);
        return index < 0
            ? string.Empty
            : FullGeneratorVariabilityMatrixVocabulary.RelativeOutputDirectory + "/" + normalized[(index + marker.Length)..];
    }
    ...
    private static FullGeneratorVariabilityDiagnostic Warning(string code, string target, string message) =>
        FullGeneratorVariabilityDiagnostic.Warning(code, target, message);
