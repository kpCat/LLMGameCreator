...
public sealed class WorldEventWeatherDayNightCrisisUnityProofRunner
{
    public static WorldEventUnityProof NotRequested(WorldEventUnityCommandPlan commandPlan) =>
        Blocked(
            "goal069.unity.not_requested",
            "Unity execution was not requested for this validation pass.",
            Summary(
                [WorldEventDiagnostic.Warning("goal069.unity.not_requested", "unity_cli", "Unity build/player proof was not requested.")],
                unityEditorExecuted: false,
                playerExecuted: false,
                commandPlan: commandPlan));

    public WorldEventUnityProof Run(
        string projectRootPath,
        string outputDirectoryPath,
        string stagingDirectoryPath,
        WorldEventUnityCommandPlan commandPlan,
        WorldEventWeatherDayNightCrisisOptions options)
    {
        ...
        {
            return Blocked(
                "goal069.unity.project_missing",
                "Repository-local Unity Alpha project or build entrypoint was not found.",
                Summary([Error("goal069.unity.project_missing", "unity/LLMGameCreatorAlpha", "Unity Alpha project and build entrypoint are required.")], false, false, commandPlan: commandPlan));
        }
        ...
        {
            return Blocked(
                "goal069.unity.unity_not_found",
                "Unity Editor executable was not found in PATH or standard Unity Hub locations.",
                Summary([Error("goal069.unity.unity_not_found", "unity_cli", "Unity Editor executable was not found.")], false, false, commandPlan: commandPlan));
        }
        ...
        diagnostics.Add(Info("goal069.unity.editor_executed", "logs/unity-build.log", "Unity Editor was invoked through the existing Alpha build entrypoint."));
        diagnostics.Add(unityResult.ExitCode == 0
            ...
            : Error("goal069.unity.editor_exit_failure", "exit_code:" + unityResult.ExitCode, "Unity Editor build process did not exit successfully."));

        if (unityResult.ExitCode != 0)
        {
            Cleanup(workRoot, options);
            return Blocked(
                "goal069.unity.editor_exit_failure",
        ...
        {
            diagnostics.Add(Error("goal069.unity.executable_missing", "build/b/LLMGameCreatorAlpha.exe", "Unity build did not produce the Alpha executable."));
            Cleanup(workRoot, options);
            return Blocked(
                "goal069.unity.executable_missing",
        ...
        diagnostics.Add(Info("goal069.unity.player_executed", "logs/alpha-player-play-loop.log", "The produced Alpha player was launched in world-event marker mode."));
        diagnostics.Add(playerResult.ExitCode == 0
            ...
            : Error("goal069.unity.player_exit_failure", "exit_code:" + playerResult.ExitCode, "Alpha player process did not exit successfully."));
        ...
        return new WorldEventUnityProof
        {
            Passed = passed,
            UnityEditorOrPlayerExecuted = true,
            BlockerCode = passed ? string.Empty : "goal069.unity.marker_or_player_failure",
            BlockerMessage = passed ? string.Empty : "Unity player did not produce all required Goal 069 world-event markers or exited with failure.",
            PlayerProof = summary with { Passed = passed },
            Diagnostics = SortDiagnostics(diagnostics.Concat(summary.Diagnostics))
        };
    }

    private static WorldEventUnityProof Blocked(string blockerCode, string blockerMessage, WorldEventUnityProofSummary summary) =>
        new()
        {
            Passed = false,
    ...
    private static WorldEventUnityProofSummary Summary(
        IEnumerable<WorldEventDiagnostic> diagnostics,
        ...
        string launchLog = "",
        string playLoopLog = "",
        WorldEventUnityCommandPlan? commandPlan = null)
    {
        ...
        foreach (var marker in missing)
        {
            summaryDiagnostics.Add(playerExecuted
                ? Error("goal069.unity.marker_missing", marker, "Unity player logs did not contain the required Goal 069 marker.")
                : Warning("goal069.unity.marker_not_checked", marker, "Unity player was not executed, so the Goal 069 marker could not be checked."));
        }

        return new WorldEventUnityProofSummary
        {
            ...
            ProvenRowCount = commandPlan?.Rows.Count(row => lines.Contains("world_event_row_completed=" + row.RowId, StringComparer.Ordinal)) ?? 0,
    ...
    private static void Cleanup(string workRoot, WorldEventWeatherDayNightCrisisOptions options)
    {
    ...
    private static string RelativeGoalPath(string path)
    {
        ...
        const string marker = "goal-069-world-event-weather-daynight-crisis-matrix/";
    ...
    private static IReadOnlyList<WorldEventDiagnostic> SortDiagnostics(IEnumerable<WorldEventDiagnostic> diagnostics) =>
        diagnostics
    ...
    private static WorldEventDiagnostic Warning(string code, string target, string message) =>
        WorldEventDiagnostic.Warning(code, target, message);
