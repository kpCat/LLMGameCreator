...
public sealed partial class OfflineGeoworldObjectiveAcceptanceRunEvidenceService
{
    ...
    {
        var directory = Resolve(root, OfflineGeoworldObjectiveAcceptanceRunVocabulary.StreamingAssetsRelativeRoot);
    ...
    private static OfflineGeoworldObjectiveNegativeScenario Scenario(
        string id,
        string mutation,
        string code,
        string target) =>
        new()
        {
            ...
            Diagnostics =
            [
                OfflineGeoworldObjectiveDiagnostic.Error(
                    code,
                    target,
                    "Goal107 negative proof rejected the mutated objective acceptance input.")
            ]
        };
    ...
    private static T? ReadSource<T>(
        string root,
        string relativePath,
        List<OfflineGeoworldObjectiveDiagnostic> diagnostics)
    {
        var path = Resolve(root, relativePath);
        if (!File.Exists(path))
        {
            diagnostics.Add(OfflineGeoworldObjectiveDiagnostic.Error(
                "goal107.read.missing",
                relativePath,
                "Required Goal107 source artifact is missing."));
            return default;
        }
    ...
    private static T? Deserialize<T>(string json) => OfflineGeoworldObjectiveJson.Deserialize<T>(json);

    private static string Serialize<T>(T value) => OfflineGeoworldObjectiveJson.Serialize(value);

    private static string Hash(string text) => OfflineGeoworldObjectiveHash.Sha256Text(text);

    private static string HashFile(string path) => OfflineGeoworldObjectiveHash.Sha256File(path);
        ...
        List<OfflineGeoworldObjectiveDiagnostic> diagnostics)
    {
        if (!condition)
        {
            diagnostics.Add(OfflineGeoworldObjectiveDiagnostic.Error(
                code,
                target,
                "Goal107 offline geoworld objective acceptance run gate did not pass."));
        }
    }

    private static IReadOnlyList<OfflineGeoworldObjectiveDiagnostic> SortDiagnostics(
        IEnumerable<OfflineGeoworldObjectiveDiagnostic> diagnostics) =>
        diagnostics
    ...
    private static string ObjectiveHash(
        string previous,
        ...
        Hash(
            ...
            + "|"
            + string.Join(",", objective.LinkedActionIds)
            + "|"
            + string.Join(",", objective.LinkedEventIds)
            + "|"
            + string.Join(",", objective.ExpectedStateDeltaKeys)
            + "|"
            + objective.ExpectedStateHashAfter);
}
