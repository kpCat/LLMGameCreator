...
public sealed class CombatMagicAbilityBossEncounterUnityProofRunner
{
    public static CombatMagicUnityProof NotRequested(CombatMagicUnityCommandPlan commandPlan) =>
        Blocked(
            "goal068.unity.not_requested",
            "Unity execution was not requested for this validation pass.",
            Summary(
                [CombatMagicDiagnostic.Warning("goal068.unity.not_requested", "unity_cli", "Unity build/player proof was not requested.")],
                unityEditorExecuted: false,
                playerExecuted: false,
                commandPlan: commandPlan));

    public CombatMagicUnityProof Run(
        ...
        CombatMagicAbilityBossEncounterOptions options)
    {
        ...
        {
            return Blocked(
                "goal068.unity.project_missing",
                "Repository-local Unity Alpha project or build entrypoint was not found.",
                Summary([Error("goal068.unity.project_missing", "unity/LLMGameCreatorAlpha", "Unity Alpha project and build entrypoint are required.")], false, false, commandPlan: commandPlan));
        }
        ...
        {
            return Blocked(
                "goal068.unity.unity_not_found",
                "Unity Editor executable was not found in PATH or standard Unity Hub locations.",
                Summary([Error("goal068.unity.unity_not_found", "unity_cli", "Unity Editor executable was not found.")], false, false, commandPlan: commandPlan));
        }
        ...
        diagnostics.Add(Info("goal068.unity.editor_executed", "logs/unity-build.log", "Unity Editor was invoked through the existing Alpha build entrypoint."));
        diagnostics.Add(unityResult.ExitCode == 0
            ? Info("goal068.unity.editor_exit_success", "exit_code:0", "Unity Editor build process exited successfully.")
            : Error("goal068.unity.editor_exit_failure", "exit_code:" + unityResult.ExitCode, "Unity Editor build process did not exit successfully."));

        if (unityResult.ExitCode != 0)
        {
            Cleanup(workRoot, options);
            return Blocked(
                "goal068.unity.editor_exit_failure",
        ...
        {
            diagnostics.Add(Error("goal068.unity.executable_missing", "build/b/LLMGameCreatorAlpha.exe", "Unity build did not produce the Alpha executable."));
            Cleanup(workRoot, options);
            return Blocked(
                "goal068.unity.executable_missing",
        ...
        diagnostics.Add(Info("goal068.unity.player_executed", "logs/alpha-player-play-loop.log", "The produced Alpha player was launched in combat/magic marker mode."));
        diagnostics.Add(playerResult.ExitCode == 0
            ? Info("goal068.unity.player_exit_success", "exit_code:0", "Alpha player process exited successfully.")
            : Error("goal068.unity.player_exit_failure", "exit_code:" + playerResult.ExitCode, "Alpha player process did not exit successfully."));
        ...
        return new CombatMagicUnityProof
        {
            Passed = passed,
            UnityEditorOrPlayerExecuted = true,
            BlockerCode = passed ? string.Empty : "goal068.unity.marker_or_player_failure",
            BlockerMessage = passed ? string.Empty : "Unity player did not produce all required Goal 068 combat/magic markers or exited with failure.",
            PlayerProof = summary with { Passed = passed },
            Diagnostics = SortDiagnostics(diagnostics.Concat(summary.Diagnostics))
        };
    }

    private static CombatMagicUnityProof Blocked(
        string blockerCode,
        string blockerMessage,
        CombatMagicUnityProofSummary summary) =>
        new()
        {
            Passed = false,
    ...
    private static CombatMagicUnityProofSummary Summary(
        IEnumerable<CombatMagicDiagnostic> diagnostics,
        ...
        string launchLog = "",
        string playLoopLog = "",
        CombatMagicUnityCommandPlan? commandPlan = null)
    {
        ...
        foreach (var marker in missing)
        {
            summaryDiagnostics.Add(playerExecuted
                ? Error("goal068.unity.marker_missing", marker, "Unity player logs did not contain the required Goal 068 marker.")
                : Warning("goal068.unity.marker_not_checked", marker, "Unity player was not executed, so the Goal 068 marker could not be checked."));
        }

        return new CombatMagicUnityProofSummary
        {
            ...
            ProvenRowCount = commandPlan?.Rows.Count(row => lines.Contains("combat_magic_row_completed=" + row.RowId, StringComparer.Ordinal)) ?? 0,
    ...
    private static void Cleanup(string workRoot, CombatMagicAbilityBossEncounterOptions options)
    {
    ...
    private static string RelativeGoalPath(string path)
    {
        ...
        var marker = "/" + CombatMagicAbilityBossEncounterVocabulary.RelativeOutputDirectory.TrimStart('/').Replace('\\', '/') + "/";
        var index = normalized.IndexOf(marker, StringComparison.OrdinalIgnoreCase);
        return index < 0
            ? string.Empty
            : CombatMagicAbilityBossEncounterVocabulary.RelativeOutputDirectory + "/" + normalized[(index + marker.Length)..];
    }

    private static IReadOnlyList<CombatMagicDiagnostic> SortDiagnostics(IEnumerable<CombatMagicDiagnostic> diagnostics) =>
        CombatMagicAbilityBossEncounterValidator.Sort(diagnostics);
    ...
    private static CombatMagicDiagnostic Warning(string code, string target, string message) =>
        CombatMagicDiagnostic.Warning(code, target, message);
