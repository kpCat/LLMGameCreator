    ...
    ICanonicalRuntimePlayerCommandLoopService
{
    ...
    public CanonicalRuntimePlayerCommandLoopService(IUnifiedGameRuntimeService runtime)
    {
        ...
        _primitiveHandlers = new Dictionary<string, Func<GamePackageDefinition, UnifiedRuntimeSession, CanonicalRuntimePlayerCommandLoopStep, UnifiedRuntimeResult>>(StringComparer.Ordinal)
        {
            ...
            ["runtime.command.use_ability"] = (package, session, step) => _runtime.ExecuteGameplayCommand(package, session,
                GameRuntimeCommand.UseAbility(Arg(step, "abilityId", step.TargetId),
                    Arg(step, "sourceParticipantId", "player"), Arg(step, "targetParticipantId", step.TargetId))),
            ["runtime.command.end_turn"] = (package, session, step) => _runtime.ExecuteGameplayCommand(package, session,
                new GameRuntimeCommand { Type = GameRuntimeCommandType.EndTurn, TargetId = step.TargetId }),
    ...
    private static string PrimitiveCommandKind(string primitiveId) => primitiveId switch
    {
        ...
        "runtime.command.use_ability" => nameof(GameRuntimeCommandType.UseAbility),
        "runtime.command.end_turn" => nameof(GameRuntimeCommandType.EndTurn),
