...
When production starts from a `.example.json` file, the production service also carries a deterministic source context from the preview into each produced artifact. The context includes the example id, title, purpose, source profile id, selected feature bundle ids, target artifact ids, and step titles by contract/id. This is data copied from the example/preview path; it is not generated by a model.
...
Known artifact payloads use the source context when it is available:

```text
game_profile_v1     uses example title/purpose for game title and description
scene_pack_v1       uses example title for the start scene
entity_pack_v1      creates player plus a deterministic guide NPC
quest_pack_v1       creates an intro quest with deterministic steps from the title/purpose terms
mechanics_pack_v1  writes id, name, title, and description
semantic_pack_v1   extracts deterministic terms from title, purpose, and bundle ids
```
...
This layer also does not generate Lua, call local or remote models, apply patches, change the Design DB schema, or change the GamePackage format. The deterministic source context improves seed quality, but it is still template-based and should not be treated as creative generation.
