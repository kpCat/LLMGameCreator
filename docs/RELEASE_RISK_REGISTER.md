...
Goal165 release-risk note: combat `CAMPAIGN_CURRENT` now accepts profile-defined BasicAttack-only, package-ability-only and both-route contracts without hiding an unavailable optional action as a failure. Defeat recovery stores only a verified in-memory pre-encounter checkpoint; it intentionally does not survive application restart or alter save schema. Runtime, public GamePackage, generated source/save migration, standalone implementation, RC and Unity remain unchanged. Rich authored combat diversity and campaign choice branching remain future work.
...
Goal168 independent audit is `BLOCKED_AT_BBFD46A2`; Goal169 closes the two P1 findings and build save-truth debt. Relationship qualification now treats absent branches as unavailable rather than failed or executed, and legacy v6 compatibility is limited to fully proven all-branch rows. Exact combat remains descriptor- and state-change-bound while accepting health, stat and status effects.

Reactive regional events are generated from actual Support completion, Challenge victory aftermath and Refusal fallout. The controlled overlay, deterministic placement, exactly-once resolution, v7 readiness, regeneration seals and fingerprint-based migration are GREEN. The single permitted cached smoke launched the standalone successfully, but Goal169 payload qualification failed because all 84 movement frames had direction-only titles and none had an explicit `Move.` prefix. The post-smoke fix emits `Move.<Direction>`, but retry remains zero; this unconsumed validation gap is the current release blocker. Goal169 is unaccepted, creates no human gate and requires independent blocker audit before planning another visible campaign system.

## Goal169A strict replay and v7 correlation closure

Goal169 remains historical `BLOCKED_AT_F861229C`; its old payload is immutable and is not promoted. Goal169A closes the release risks behind that audit: replay equivalence compares every typed route dimension, v7 rejects graph/hash/semantic tampering, Challenge placement follows exact encounter-region provenance, and migration compatibility consumes exact source/target v7 inventories plus definition fingerprints. The separate one-shot post-fix payload proves explicit `Move.*` with zero bare directions. Residual process risk is independent audit only; Goal169A is unaccepted and creates no human gate.
