...
Goal163 release-risk note: the campaign workspace no longer clones or patches the qualified package at action time. Exact package dispatch, command identity, read-only quest readiness and human consequence rows are enforced. Release remains blocked because generated current-region quest encounters do not expose a Runtime-executable damage route: their generated health/resource and effectless generated action do not match existing BasicAttack/UseAbility contracts. The UI now reports the encounter as unavailable instead of claiming synthetic combat, victory or manual quest completion. Runtime, GamePackage, generated save/migration, Unity and standalone/RC implementations remain unchanged; Player/Unity/standalone invocation counts are zero.
