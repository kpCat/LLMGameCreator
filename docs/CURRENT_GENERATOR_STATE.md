...
Updated by: Goal 032 dynamic semantic feature system  
...
1. Review `dynamic_semantic_feature_system_verification` evidence from the Goal 032 feature catalog summary, influence rule summary, authoring schema matrix, four resolved state files, focused tests, product smoke and invalid/fake/leak evidence before marking the gate passed.
2. Confirm the report keeps `accepted=false` and the marker `dynamic_semantic_feature_system_verification required`.
3. Do not start Goal 033 until the Goal 032 gate is accepted by the user.
...
Goal 032 stops at `dynamic_semantic_feature_system_verification` required, not passed. Goal 031 also remains produced for review and not marked passed by this handoff. Goal 033 is not started.
