...
Updated by: Goal 018 Unity multi-variant playable scenario  
...
1. Review `unity_generated_multi_variant_playable_scenario_verification` evidence from the produced Unity multi-variant playable scenario variants JSON, report, Windows players, Unity build logs, per-variant launch/play-loop logs, distinct style/package/thread/quest/scene/objective/command identities, reward proof and invalid/fake/leak matrix before marking the gate passed.
...
The active gate is now `unity_generated_multi_variant_playable_scenario_verification` and remains required, not passed.
...
After Goal 018, this state should recommend:
