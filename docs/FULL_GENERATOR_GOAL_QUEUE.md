...
```text
candidate package -> package validation -> canonical runtime playthrough -> save/load/replay proof -> player adapter contract -> player-loop readiness plan -> Unity/player readiness smoke -> one-click report
```

Review Goal135 evidence before starting a new implementation slice. If accepted,
the next bounded product step can build an actual playable player loop on the
...
```text
canonical_runtime_playable_player_loop_readiness_verification_required_accepted_false
```
