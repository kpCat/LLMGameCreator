...
- Goal 164 generated encounter combat contract and campaign qualification: `GREEN_ACCEPTABLE_CANDIDATE`, `accepted=false`, no manual gate, independent audit required. Exact Runtime-qualified Lane A supplies the deterministic combat contract; strict-provenance Lane B changes only generated participant combat fields. v4 history is `CAMPAIGN_CURRENT`; old v3 is only `COMBAT_PENDING`. Actual all-selectable/core-only campaign, regeneration, rollback, migration, one cache-only standalone smoke, RC/portable recovery and immutable sources are GREEN. Next action is `independent_goal164_audit_and_plan_campaign_choice_branching_or_failure_recovery`.
