...
- Goal 161S immutable standalone publication closure: `BLOCKED_RELEASE_CANDIDATE_RECORD_AFTER_GREEN_IMMUTABLE_PUBLICATION`, `accepted=false`, no human gate. The one new player smoke assembled and executed `%LOCALAPPDATA%/LGC/O/<token>/runs/r-<attempt>` directly, passed preflight/markers/Player.log with exit 0, wrote GREEN run status and atomically validated `current.json`; host reuse=true, host rebuild=false and Unity=0. The controller then returned exact diagnostic `rc.payload.missing` at `release_candidate_record`; retry=0. Next work is independent RC payload-resolution diagnosis without player retry, Unity or host rebuild.
