...
- Goal 169 profile-neutral relationships and reactive regional events: `BLOCKED_AFTER_SINGLE_HIDDEN_SMOKE_MOVE_FRAME_ASSERTION`, `accepted=false`, no manual gate, independent audit required. It closes Goal168 audit `BLOCKED_AT_BBFD46A2`: branch qualification/history now follow actual profile availability, exact combat accepts health/stat/status progress, and build save truth is honest. Data-derived Support/Challenge/Refuse events add deterministic reachable placement, exactly-once Runtime resolution, «События мира», v7 readiness, sealed regeneration and exact save/migration. The only cached smoke launched the standalone successfully with CURRENT RC, host reuse, 5/5 self-checks and zero Unity starts, but Goal169 payload qualification rejected its direction-only movement frame titles. The post-smoke `Move.<Direction>` fix is implemented; retry remains zero.
