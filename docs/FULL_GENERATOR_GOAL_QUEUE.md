...
- Goal 161R short player output path and standalone qualification closure: `BLOCKED_PUBLICATION_AFTER_GREEN_SHORT_PATH_SMOKE`, `accepted=false`, no human gate. The short `%LOCALAPPDATA%/LGC/O/<token>/s-<attempt>` staging output and 240-character budget remove the proven 260-character player-path failure: the one new smoke exited 0 with all five markers and Player.log, reused cache `6af4d5eb5b42f956110555b58fb4e276`, rebuilt no host and started Unity zero times. Publication then failed and rolled staging back; corrective retry count is zero. Next work is independent publication diagnosis without player retry, Unity or host rebuild.
