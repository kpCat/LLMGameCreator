...
- Goal 161Q standalone diagnosis and qualification closure: `BLOCKED_PLAYER_PAYLOAD_PATH_LENGTH_260`, `accepted=false`, no human gate. Application now performs 13 named structural/package-SHA checks plus exact legacy parser compatibility before publishing/launching and captures a separate bounded Player.log. The one new smoke reused cache `6af4d5eb5b42f956110555b58fb4e276`, rebuilt no host and started Unity zero times, but exit code 2 named the exact 260-character `player-adapter-model.json` path failure. Corrective retry count is zero. Next work requires a new explicit budget for a short confined Application smoke path; do not change Unity or start the next product slice.
