...
- Goal 163 package-truth campaign combat, manual turn-in and consequence depth: `BLOCKED_EXACT_GENERATED_ENCOUNTER_PACKAGE_CONTRACT`, `accepted=false`, no human gate. Synthetic package mutation and fixed combat power are removed; exact package reference/SHA/inventory, unchanged BasicAttack, participant-owned abilities, read-only quest readiness, one manual CompleteQuest, typed history truth and human consequences are enforced. The real ordinary sample encounter executes, but current-region generated quest encounters have no Runtime-executable combat contract, so real generated victory/turn-in and post-turn-in save/migration proof remain blocked. Next action is a separate generated encounter package-contract repair, then repeat the real Goal163 matrix.
