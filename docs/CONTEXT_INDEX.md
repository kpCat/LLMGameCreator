...
| `docs/GENERATOR_PLAN_CAPABILITY_SELECTION_PICKER.md` | Building and saving the M3 capability/feature bundle selection artifact before later generator plans, prompts, Lua modules or assembly tasks. |
| `docs/GENERATOR_PLAN_STRICT_LLM_ARTIFACT_GENERATION.md` | Running M4 strict LLM generation from the latest capability selection through exact-JSON parsing, C# validation, bounded repair, audit persistence and pending Artifact Review staging. |
| `docs/GENERATOR_PLAN_STRICT_LLM_EVALUATION.md` | Evaluating strict LLM generation audits and explicit small batches before expanding contracts or Lua generation. |
...
- Capability selection uses `GeneratorPlanCapabilitySelectionService` and `GeneratorPlanCapabilitySelectionArtifactService` to resolve taxonomy ids, feature bundles, contracts, validators, runtime targets, prompt context templates and gaps into `artifact/generator_plan_capability_selection/latest`. It does not call providers, execute Lua, mutate `GamePackage`, export packages or change Design DB schema.
- Strict LLM artifact generation uses the latest capability selection and produces one contract-bound JSON artifact per LLM call. Read `docs/GENERATOR_PLAN_STRICT_LLM_ARTIFACT_GENERATION.md`, `GeneratorPlanStrictLlmArtifactGenerationService`, strict parser/validator files and the Artifact Review services when touching this flow. It calls `ILlmChatClient` only from explicit editor generation, stages valid artifacts as pending review items, saves audit artifacts, and does not mutate `GamePackage`.
- Strict LLM evaluation uses `GeneratorPlanStrictLlmEvaluationService` over the latest strict generation audit or explicit small batches through the existing strict generation service. Read `docs/GENERATOR_PLAN_STRICT_LLM_EVALUATION.md` when touching pass/repair/fail metrics, quality heuristics, evaluation artifacts, markdown reports or the `LLM Evaluation` page. Latest-audit evaluation must not call an LLM; batch evaluation must be user-triggered and use fake `ILlmChatClient` in tests.
