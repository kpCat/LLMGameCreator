...
Goal163 is the current truth-repair slice and is `BLOCKED_EXACT_GENERATED_ENCOUNTER_PACKAGE_CONTRACT`; `goal163Accepted=false`, no human gate. Goal162 independent audit is `BLOCKED_AT_8164185B`. Goal163 removes the synthetic package clone/fixed attack and preserves exact package references, BasicAttack command identity, read-only generated quest readiness, manual CompleteQuest semantics, typed final-state/history truth and a human `Последствия` ledger. The real package can execute its ordinary sample encounter, but current-region generated quest encounters expose `generated/resource/health` plus an effectless `generated_action`; they are now causally disabled, so the generated victory/turn-in/save/migration real matrix cannot pass. Next action is a separately scoped generated encounter package-contract repair, then repeat Goal163.
