# Goal166 exact combat actions and real recovery

Goal166 is a GREEN acceptable automated candidate. It is not accepted by a human or Codex, has no manual gate and requires an independent audit.

The combat contract now retains exact qualified descriptors: BasicAttack and only package abilities that produced a supported target health/stat/status delta. A utility/no-op action remains available as support, but cannot enter the victory catalog or be selected by representative victory. The v4 summary and candidate seal hash the ordered catalog.
