...
Status: `locked_by_M8_004`

Depends on:

```text
- M8_004 completed;
- runtime preview scenario has deterministic success and fail cases.
```

Unlocks:

```text
- stop or next gate decision based on runtime preview evidence.
```
...
```text
User approval: approved to start M8_005 after package assembly path exists
```
...
```text
Produce a deterministic runtime preview diagnostic report for failed package load/command/snapshot scenarios.
```
...
```text
- failed preview returns structured diagnostics/report;
- report includes package/scenario id, diagnostic code, message, and suggested next step when available;
...
```text
- invalid package report includes runtime.preview.package.invalid;
- invalid command report includes runtime.preview.command.invalid;
- snapshot mismatch report includes runtime.preview.snapshot.mismatch;
- report ordering is deterministic;
- no LLM/provider repair is invoked.
```
...
```text
- report format requires broad public contract change;
- failures cannot be represented without UI/provider calls;
- report duplicates existing diagnostic/report pattern instead of reusing it;
...
Task id: M9_001
Task spec file: docs/agent-tasks/M9/M9_001_TEMPLATE_CONTRACTS.md
Reason: Only after current-state docs unlock M9; otherwise stop for user review.
```

On block, write BLOCKERS:

```text
M8_005 blocked: package assembly path is missing, current gate is not open, or runtime boundary assumptions need architecture approval.
```
