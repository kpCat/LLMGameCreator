...
Status: `locked_by_M8_001`

Depends on:

```text
- M8_001 completed;
- a loaded validated package fixture exists;
- runtime command entrypoint is identified.
```
...
```text
User approval: approved to start M8_002 after package assembly path exists
```
...
```text
Run one small deterministic command sequence against a loaded package and verify final state/events.
```
...
```text
- same package + same command sequence produces the same final state/events;
- invalid command returns deterministic diagnostic;
- no renderer/UI path is required.
```
...
```text
- valid command sequence succeeds and returns expected final state/event count;
- same sequence repeated twice yields equivalent state/events;
- invalid command returns runtime.preview.command.invalid;
- scenario does not mutate source package definition.
```
...
```text
- deterministic command entrypoint is missing;
- command sequence requires UI input/event loop;
- runtime state cannot be compared deterministically;
...
Task id: M8_003
Task spec file: docs/agent-tasks/M8/M8_003_SNAPSHOT_GUARD.md
Reason: Add snapshot/golden guard after deterministic command scenario is proven.
```

On block, write BLOCKERS:

```text
M8_002 blocked: package assembly path is missing, current gate is not open, or runtime boundary assumptions need architecture approval.
```
