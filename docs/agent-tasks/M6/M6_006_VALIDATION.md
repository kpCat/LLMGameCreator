# M6_006 — Package validation after assembly
...
```text
M6_007 review/apply boundary after validation is proven.
```
...
```text
User approval: approved to start M6_006 after M4.1/M5 gates and M6 prerequisites are satisfied
```
...
tests/LLMGameCreator.Tests/Design/**
tests/LLMGameCreator.Tests/Validation/**
tests/fixtures/artifact_mapping/assembled/**
...
```text
Add or harden the validation gate that runs after package assembly dry-run/apply. This task should ensure assembled package fragments are validated together, not just per-family.
```
...
```text
artifact_mapping.assembled.validation_failed
artifact_mapping.assembled.ref.invalid
artifact_mapping.assembled.id.duplicate
artifact_mapping.assembled.required_missing
```
...
```text
- valid multi-family assembled fixture passes package validation;
- missing cross-family ref -> artifact_mapping.assembled.ref.invalid;
- duplicate id across assembled fragments -> artifact_mapping.assembled.id.duplicate;
- missing required package section -> artifact_mapping.assembled.required_missing;
- diagnostics preserve target path/id.
```
...
Task id: M6_007
Reason: Continue M6 assembly sequence after M6_006 passes.
```
