# M6_002 — Artifact envelope to package base mapping
...
```text
M6_003 items/economy mapping after base envelope mapping seam is proven.
```
...
```text
User approval: approved to start M6_002 after M4.1/M5 gates and M6 prerequisites are satisfied
```
...
tests/fixtures/artifact_mapping/base/**
...
```text
Map a small approved artifact envelope into a package-level dry-run/patch result that can be validated before apply. This task should prove the base mapping seam, not implement item/scene/quest family mapping.
```
...
```text
artifact_mapping.base.contract.unsupported
artifact_mapping.base.patch.invalid
artifact_mapping.base.package.validation_failed
artifact_mapping.base.schema.required
```
...
```text
- valid base artifact envelope maps to exact expected patch/dry-run result;
- unsupported contract id -> artifact_mapping.base.contract.unsupported;
- invalid base package fragment -> artifact_mapping.base.patch.invalid;
- resulting package dry-run passes GamePackageValidator for valid fixture;
- Runtime/UI/provider/Lua projects are not touched.
```
...
Task id: M6_003
Reason: Continue M6 assembly sequence after M6_002 passes.
```
