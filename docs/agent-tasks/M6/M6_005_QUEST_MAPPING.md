# M6_005 — Dialogue and quest mapping
...
```text
M6_006 package validation after assembly once multiple families can map.
```
...
```text
User approval: approved to start M6_005 after M4.1/M5 gates and M6 prerequisites are satisfied
```
...
tests/fixtures/artifact_mapping/quests/**
...
```text
Map one reviewed dialogue/quest artifact family into package dialogue/quest fragments. This task must reject unresolved scene/item/NPC refs and preserve review/apply boundary.
```
...
```text
artifact_mapping.quest.id.missing
artifact_mapping.quest.id.duplicate
artifact_mapping.quest.ref.invalid
artifact_mapping.dialogue.ref.invalid
artifact_mapping.quest.package.validation_failed
```
...
```text
- valid quest/dialogue artifact creates exact expected patch fragment;
- duplicate quest/dialogue id -> exact duplicate diagnostic;
- invalid scene/item/NPC ref -> exact ref diagnostic;
- deterministic quest step ordering is pinned;
...
Task id: M6_006
Reason: Continue M6 assembly sequence after M6_005 passes.
```
