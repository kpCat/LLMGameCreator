# M6_003 — Items and economy mapping
...
```text
M6_004 scene/map mapping after item/economy mapping is guarded.
```
...
```text
User approval: approved to start M6_003 after M4.1/M5 gates and M6 prerequisites are satisfied
```
...
tests/fixtures/artifact_mapping/items/**
...
```text
Map one reviewed items/economy artifact family into package item/economy fragments through the M6_001/M6_002 dry-run boundary. Keep this to one artifact family and one small fixture set.
```
...
```text
artifact_mapping.items.id.missing
artifact_mapping.items.id.duplicate
artifact_mapping.items.ref.invalid
artifact_mapping.items.value.invalid
artifact_mapping.items.package.validation_failed
```
...
```text
- valid item artifact creates exact expected item/economy patch fragment;
- duplicate item id -> artifact_mapping.items.id.duplicate;
- invalid item/economy reference -> artifact_mapping.items.ref.invalid;
- invalid numeric/value field -> artifact_mapping.items.value.invalid;
- package validator passes after valid dry-run.
```
...
Task id: M6_004
Reason: Continue M6 assembly sequence after M6_003 passes.
```
