...
```text
M6_005 dialogue/quest mapping after scene/map references are stable.
```
...
```text
User approval: approved to start M6_004 after M4.1/M5 gates and M6 prerequisites are satisfied
```
...
tests/fixtures/artifact_mapping/scenes/**
...
```text
Map one reviewed scene/map artifact family into package scene/map fragments with deterministic ids, references, and ordering. Keep movement/runtime behavior out of this task.
```
...
```text
artifact_mapping.scene.id.missing
artifact_mapping.scene.id.duplicate
artifact_mapping.scene.ref.invalid
artifact_mapping.scene.exit.invalid
artifact_mapping.scene.package.validation_failed
```
...
```text
- valid scene/map artifact creates exact expected scene/map patch fragment;
- duplicate scene id -> artifact_mapping.scene.id.duplicate;
- invalid exit/target ref -> artifact_mapping.scene.exit.invalid;
- deterministic ordering is pinned in expected output;
...
Task id: M6_005
Reason: Continue M6 assembly sequence after M6_004 passes.
```
