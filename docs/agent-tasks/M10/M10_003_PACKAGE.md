# M10_003 — Deterministic export package
...
- M10_001 completed;
- M10_002 completed;
- current-state docs explicitly unlock deterministic export package work.
```
...
```text
User approval: approved to start M10_003 after M10 gate is unlocked
```
...
```text
src/LLMGameCreator.Runtime/** unless explicit boundary tests require read-only inspection
...
```text
Create a deterministic export package artifact from validated package/profile/IR inputs without mutating source package or invoking providers.
```

Expected behavior after unlock:

```text
- export package has deterministic manifest/order/content hashes;
- same input -> same output;
- invalid input -> diagnostic and no package output;
- source GamePackage remains unchanged.
```

Failure behavior / diagnostics:

```text
export.package.input.invalid
export.package.hash.mismatch
export.package.non_deterministic
export.package.asset.missing
```
...
```text
- same fixture exported twice -> identical manifest/hash/order;
- missing asset ref -> export.package.asset.missing;
- invalid IR -> export.package.input.invalid;
- source package object/file unchanged after export.
```
...
```text
- deterministic output cannot be pinned;
- export requires mutating source package;
- export requires provider/network call;
- asset paths are unclear;
...
Task id: M10_004
Task spec file: docs/agent-tasks/M10/M10_004_BOUNDARY.md
Reason: Prove player/runtime boundary after deterministic export package exists.
```

On block, write BLOCKERS:

```text
M10_003 blocked: M10 gate is not unlocked or source assumptions require architecture approval.
```
