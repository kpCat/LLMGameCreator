...
Goal165 gate note: Goal164 implementation remains GREEN and unaccepted; its independent-audit P1 is recorded as `BLOCKED_AT_15A8F2AB` and closed by typed route-neutral qualification/history truth. `BASIC_ATTACK_ONLY`, `PACKAGE_ABILITY_ONLY` and `BOTH` are valid `CAMPAIGN_CURRENT` profiles. The campaign now exposes a nonpersistent exact checkpoint recovery after genuine defeat; stale project truth is rejected before Runtime dispatch. Goal165 remains `accepted=false`, creates no manual gate and requires independent audit.
