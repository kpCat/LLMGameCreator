...
Goal164 gate note: the Goal163 generated-combat blocker at `d5d614a8` is closed by a deterministic build-time contract taken only from the exact Runtime-qualified Lane A package. Strict plan plus `GeneratedContent` provenance binds every generated encounter; only participant combat fields change in Lane B. Actual Runtime campaign, v4 `CAMPAIGN_CURRENT`, regeneration/rollback, migration, one cached hidden smoke and portable all/core are GREEN. Goal164 remains `accepted=false`, creates no manual gate and requires independent audit.
