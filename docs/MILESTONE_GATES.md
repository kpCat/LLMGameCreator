...
Goal153, Goal153A, Goal153B and Goal153C implementations are GREEN and `manualGateReady=true`: the three default-off ability/mana/turn-status FeatureModules contain no activated proof participant or global health-capacity mutation, select the real hostile deterministically, preserve rollback/domain contracts and accept truthful expiry/defeat/encounter outcomes. Defaults `2/12/3/5/1` produce five ticks and expiry; lethal paths skip remaining conditional EndTurns without mutation and replay identically. The active gate is `independent_goal153c_audit_then_combined_goal153_family_human_gate required`; no Goal153-family human acceptance is claimed.
