...
public sealed class OfflineGeoworldUnityPreviewRunnerProductSmokeTests
{
    ...
    [Fact]
    public async Task Goal101OfflineGeoworldUnityPreviewRunnerProductSmoke()
    {
        ...
        Assert.True(result.Report.SimulatedCommandProofPassed);
        ...
        using var commands = JsonDocument.Parse(await File.ReadAllTextAsync(
            Path.Combine(write.StreamingAssetsDirectoryPath,
                OfflineGeoworldUnityPreviewRunnerVocabulary.FeatureCommandsFileName)));
        using var travel = JsonDocument.Parse(await File.ReadAllTextAsync(
            Path.Combine(write.StreamingAssetsDirectoryPath,
                OfflineGeoworldUnityPreviewRunnerVocabulary.TravelWindowScriptFileName)));
        using var style = JsonDocument.Parse(await File.ReadAllTextAsync(
            Path.Combine(write.StreamingAssetsDirectoryPath,
                OfflineGeoworldUnityPreviewRunnerVocabulary.StyleLegendFileName)));

        Assert.Equal(18, manifest.RootElement.GetProperty("commandCount").GetInt32());
        Assert.Equal(10, manifest.RootElement.GetProperty("commandKindCount").GetInt32());
        ...
        Assert.False(manifest.RootElement.GetProperty("containsFinalArt").GetBoolean());
        Assert.Equal(18, commands.RootElement.GetProperty("commandCount").GetInt32());
        Assert.Equal(10, commands.RootElement.GetProperty("commandKindCount").GetInt32());
        Assert.True(travel.RootElement.GetProperty("stepCount").GetInt32() >= 4);
        Assert.Equal(10, style.RootElement.GetProperty("styleCount").GetInt32());

        foreach (var kind in OfflineGeoworldUnityPreviewRunnerVocabulary.RequiredCommandKinds)
        {
            Assert.True(
                result.SimulatedCommandProof.CommandCountByKind.ContainsKey(kind),
                kind);
        }
        ...
        Assert.All(result.UnityScriptInventory.Files, file =>
        {
            ...
            Assert.True(file.HasNoProviderLlmNetworkMarkers, file.RelativePath);
        ...
        Assert.True(workspace.QualityGateScan.OfflineGeoworldUnityPreviewGroupPresent);
        Assert.Equal(18, workspace.QualityGateScan.OfflineGeoworldUnityPreviewCommandCount);
        Assert.True(workspace.QualityGateScan.OfflineGeoworldUnityPreviewUnityScriptsReady);
        Assert.True(workspace.QualityGateScan.Goal101FilesDiscoveredByRelativePaths);
        ...
        Assert.Contains("offline_geoworld_unity_preview_runner_verification required", report);
        Assert.Contains("commandCount: 18", report);
        Assert.Contains("travelWindowStepCount:", report);
