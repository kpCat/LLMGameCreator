...
public sealed class QuestDialogInteractionFamilySmokeTests
{
    [Fact]
    public async Task QuestDialogInteractionFamilyProductSmoke()
    {
        using var temp = new TempDirectory();
        var projectRoot = ResolveProjectFolder(temp.Path);
        var serializer = new RuntimeStateSerializer();
        var service = new QuestDialogInteractionFamilyAcceptanceService(
        ...
        Assert.True(result.Report.DialogueEvidenceGenerated);
        Assert.True(result.Report.InteractionFamilyInvocationCount >= 2);
        ...
        Assert.Contains("\"code\": \"quest_dialog_interaction.no_external_execution\"", json);
    }
