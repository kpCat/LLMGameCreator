...
public sealed class GeneratedMicrogameLoopSmokeTests
{
    [Fact]
    public async Task GeneratedMicrogameLoopProductSmoke()
    {
        ...
        var result = await service.ExecuteAsync(new OneClickGeneratedPreviewWorkflowRequest
        {
            ...
            Seed = "product-smoke-generated-microgame-loop",
        ...
        Assert.True(challenge.ChallengeSelected);
        ...
        Assert.True(File.Exists(result.Paths.MicrogameAcceptanceReportMarkdownPath));
        Assert.True(File.Exists(result.Paths.MicrogameManualVerificationMarkdownPath));
        Assert.Contains("generated-microgame-loop", result.Paths.MicrogameAcceptanceSnapshotJsonPath, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("generated_microgame_acceptance.no_external_execution", File.ReadAllText(result.Paths.MicrogameAcceptanceSnapshotJsonPath));
        Assert.Contains("Manual Microgame Loop Verification", File.ReadAllText(result.Paths.MicrogameManualVerificationMarkdownPath));
    }
