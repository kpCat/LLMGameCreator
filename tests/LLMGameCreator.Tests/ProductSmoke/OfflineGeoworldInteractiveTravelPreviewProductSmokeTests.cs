...
public sealed class OfflineGeoworldInteractiveTravelPreviewProductSmokeTests
{
    ...
    [Fact]
    public async Task Goal104OfflineGeoworldInteractiveTravelPreviewProductSmoke()
    {
        ...
        Assert.Equal(6, result.Report.MovementSampleCount);
        Assert.Equal(2, result.Report.BoundaryCrossingCount);
        Assert.Equal(2, result.Report.PrefetchPlanCount);
        ...
        Assert.True(result.Report.WorkspaceBindingPassed);
        ...
        using var movement = JsonDocument.Parse(await File.ReadAllTextAsync(
            Path.Combine(write.StreamingAssetsDirectoryPath,
                OfflineGeoworldInteractiveTravelPreviewVocabulary.StepsFileName)));
        using var zones = JsonDocument.Parse(await File.ReadAllTextAsync(
            Path.Combine(write.StreamingAssetsDirectoryPath,
                OfflineGeoworldInteractiveTravelPreviewVocabulary.ChunkVisibilityFileName)));
        using var prefetch = JsonDocument.Parse(await File.ReadAllTextAsync(
            Path.Combine(write.StreamingAssetsDirectoryPath,
        ...
        Assert.Equal(6, manifest.RootElement.GetProperty("movementSampleCount").GetInt32());
        Assert.Equal(2, manifest.RootElement.GetProperty("boundaryCrossingCount").GetInt32());
        Assert.Equal(2, manifest.RootElement.GetProperty("prefetchPlanCount").GetInt32());
        ...
        Assert.Equal(6, movement.RootElement.GetProperty("movementSampleCount").GetInt32());
        Assert.Equal(2, zones.RootElement.GetProperty("boundaryCrossingCount").GetInt32());
        Assert.Equal(2, prefetch.RootElement.GetProperty("prefetchPlanCount").GetInt32());
        Assert.Equal(18, prefetch.RootElement.GetProperty("objectCount").GetInt32());
        ...
        Assert.True(workspace.QualityGateScan.OfflineGeoworldInteractiveTravelGroupPresent);
        Assert.Equal(6, workspace.QualityGateScan.OfflineGeoworldInteractiveTravelMovementSampleCount);
        Assert.Equal(2, workspace.QualityGateScan.OfflineGeoworldInteractiveTravelBoundaryCrossingCount);
        Assert.Equal(18, workspace.QualityGateScan.OfflineGeoworldInteractiveTravelObjectCount);
        Assert.True(workspace.QualityGateScan.OfflineGeoworldInteractiveTravelUnityScriptsReady);
        Assert.True(workspace.QualityGateScan.OfflineGeoworldInteractiveTravelEditorWindowReady);
        Assert.True(workspace.QualityGateScan.Goal104FilesDiscoveredByRelativePaths);
        ...
        Assert.Contains("offline_geoworld_interactive_travel_preview_verification required", report);
        Assert.Contains("movementSampleCount: 6", report);
        Assert.Contains("boundaryCrossingCount: 2", report);
        Assert.Contains("prefetchPlanCount: 2", report);
