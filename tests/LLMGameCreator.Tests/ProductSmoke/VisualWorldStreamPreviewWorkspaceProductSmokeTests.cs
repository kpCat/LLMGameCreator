...
public sealed class VisualWorldStreamPreviewWorkspaceProductSmokeTests
{
    ...
    public async Task Goal096UnityHandoffInspectorProbeReadinessProductSmoke()
    {
        ...
        await new OfflineGeoworldAlphaSliceExportPackageEvidenceService()
            .BuildAndWriteAsync(repoRoot);
        await new OfflineGeoworldAlphaSliceManualAcceptanceGateEvidenceService()
            .BuildAndWriteAsync(repoRoot);
        ...
        var sessionGroup = Assert.Single(
            groups,
            item => item.GetProperty("groupId").GetString() == "offline_geoworld_session_replay");
        var sessionEntries = sessionGroup.GetProperty("entries").EnumerateArray().ToArray();
        var sessionSummary = Assert.Single(
            sessionEntries,
            item => item.GetProperty("artifactKind").GetString()
                    == "offline_geoworld_session_workspace_summary");
        var sessionPayloads = sessionEntries
            .Where(item =>
                item.GetProperty("artifactKind").GetString()
                == "offline_geoworld_session_streamingassets_payload")
            .ToArray();
        var sessionScripts = sessionEntries
            .Where(item =>
                item.GetProperty("artifactKind").GetString()
                == "offline_geoworld_session_unity_script")
            .ToArray();
        var sessionEditorScript = Assert.Single(
            sessionEntries,
            item => item.GetProperty("artifactKind").GetString()
                    == "offline_geoworld_session_editor_window_script");
        Assert.True(sessionSummary.GetProperty("offlineGeoworldSessionReplayStepCount").GetInt32() >= 6);
        Assert.True(sessionSummary.GetProperty("offlineGeoworldSessionStateDeltaCount").GetInt32() >= 6);
        Assert.True(sessionSummary.GetProperty("offlineGeoworldSessionCheckpointStepIndex").GetInt32() >= 3);
        Assert.True(sessionSummary.GetProperty("offlineGeoworldSessionAcceptanceChecklistStepCount").GetInt32() > 0);
        Assert.Equal(6, sessionPayloads.Length);
        Assert.Equal(3, sessionScripts.Length);
        Assert.True(sessionEditorScript.GetProperty("offlineGeoworldSessionEditorWindowReady").GetBoolean());
        Assert.True(sessionSummary.GetProperty("offlineGeoworldSessionUnityScriptsReady").GetBoolean());
        Assert.True(sessionSummary.GetProperty("offlineGeoworldSessionEditorWindowReady").GetBoolean());
        Assert.True(sessionSummary.GetProperty("offlineGeoworldSessionSimulatedReplayProofPassed").GetBoolean());
        Assert.True(sessionSummary.GetProperty("offlineGeoworldSessionNegativeProofPassed").GetBoolean());
        Assert.True(sessionSummary.GetProperty("offlineGeoworldSessionAlphaRuntimeBootstrapUnchanged").GetBoolean());
        Assert.True(sessionSummary.GetProperty("offlineGeoworldSessionQualityGatePassed").GetBoolean());
        var objectiveGroup = Assert.Single(
            groups,
            item => item.GetProperty("groupId").GetString() == "offline_geoworld_objective_acceptance");
        var objectiveEntries = objectiveGroup.GetProperty("entries").EnumerateArray().ToArray();
        var objectiveSummary = Assert.Single(
            objectiveEntries,
            item => item.GetProperty("artifactKind").GetString()
                    == "offline_geoworld_objective_workspace_summary");
        var objectivePayloads = objectiveEntries
            .Where(item =>
                item.GetProperty("artifactKind").GetString()
                == "offline_geoworld_objective_streamingassets_payload")
            .ToArray();
        var objectiveScripts = objectiveEntries
            .Where(item =>
                item.GetProperty("artifactKind").GetString()
                == "offline_geoworld_objective_unity_script")
            .ToArray();
        Assert.True(objectiveSummary.GetProperty("offlineGeoworldObjectiveCount").GetInt32() >= 6);
        Assert.Equal(
            objectiveSummary.GetProperty("offlineGeoworldObjectiveCount").GetInt32(),
            objectiveSummary.GetProperty("offlineGeoworldObjectiveCompletedCount").GetInt32());
        Assert.Equal(6, objectivePayloads.Length);
        Assert.Equal(3, objectiveScripts.Length);
        Assert.True(objectiveSummary.GetProperty("offlineGeoworldObjectiveReplayAcceptanceProofPassed").GetBoolean());
        Assert.True(objectiveSummary.GetProperty("offlineGeoworldObjectiveAlphaQualityConsolidationPassed").GetBoolean());
        Assert.True(objectiveSummary.GetProperty("offlineGeoworldObjectiveQualityGatePassed").GetBoolean());
        var alphaSliceGroup = Assert.Single(
            groups,
            item => item.GetProperty("groupId").GetString() == "offline_geoworld_alpha_slice");
        var alphaSliceEntries = alphaSliceGroup.GetProperty("entries").EnumerateArray().ToArray();
        var alphaSliceSummary = Assert.Single(
            alphaSliceEntries,
            item => item.GetProperty("artifactKind").GetString()
                    == "offline_geoworld_alpha_slice_workspace_summary");
        var alphaSlicePayloads = alphaSliceEntries
            .Where(item =>
                item.GetProperty("artifactKind").GetString()
                == "offline_geoworld_alpha_slice_streamingassets_payload")
            .ToArray();
        var alphaSliceUnityScripts = alphaSliceEntries
            .Where(item =>
                item.GetProperty("artifactKind").GetString()
                is "offline_geoworld_alpha_slice_coordinator_script"
                or "offline_geoworld_alpha_slice_editor_window_script")
            .ToArray();
        Assert.Equal(7, alphaSliceSummary.GetProperty("offlineGeoworldAlphaSliceComponentCount").GetInt32());
        Assert.Equal(7, alphaSliceSummary.GetProperty("offlineGeoworldAlphaSliceReadyComponentCount").GetInt32());
        Assert.True(alphaSliceSummary.GetProperty("offlineGeoworldAlphaSliceObjectiveCount").GetInt32() >= 5);
        Assert.Equal(
            alphaSliceSummary.GetProperty("offlineGeoworldAlphaSliceObjectiveCount").GetInt32(),
            alphaSliceSummary.GetProperty("offlineGeoworldAlphaSliceCompletedObjectiveCount").GetInt32());
        Assert.Equal("completed", alphaSliceSummary.GetProperty("offlineGeoworldAlphaSliceFinalStatus").GetString());
        Assert.Equal(5, alphaSlicePayloads.Length);
        Assert.Equal(2, alphaSliceUnityScripts.Length);
        Assert.True(alphaSliceSummary.GetProperty("offlineGeoworldAlphaSliceUnityToolReady").GetBoolean());
        Assert.True(alphaSliceSummary.GetProperty("offlineGeoworldAlphaSliceAcceptanceRunbookReady").GetBoolean());
        Assert.True(alphaSliceSummary.GetProperty("offlineGeoworldAlphaSliceFinalProofPassed").GetBoolean());
        ...
        var manualAcceptanceGroup = Assert.Single(
            groups,
            item => item.GetProperty("groupId").GetString() == "offline_geoworld_alpha_manual_acceptance");
        var manualAcceptanceEntries = manualAcceptanceGroup.GetProperty("entries").EnumerateArray().ToArray();
        var manualAcceptanceSummary = Assert.Single(
            manualAcceptanceEntries,
            item => item.GetProperty("artifactKind").GetString()
                    == "offline_geoworld_alpha_manual_acceptance_workspace_summary");
        Assert.Equal(
            12,
            manualAcceptanceSummary
                .GetProperty("offlineGeoworldAlphaManualAcceptanceChecklistStepCount")
                .GetInt32());
        Assert.Equal(
            5,
            manualAcceptanceSummary
                .GetProperty("offlineGeoworldAlphaManualAcceptancePayloadFileCount")
                .GetInt32());
        Assert.Equal(
            7,
            manualAcceptanceSummary
                .GetProperty("offlineGeoworldAlphaManualAcceptanceExportFileCount")
                .GetInt32());
        Assert.True(manualAcceptanceSummary
            .GetProperty("offlineGeoworldAlphaManualAcceptanceManualPending")
            .GetBoolean());
        Assert.True(manualAcceptanceSummary
            .GetProperty("offlineGeoworldAlphaManualAcceptanceUnityRunnerReady")
            .GetBoolean());
        Assert.True(manualAcceptanceSummary
            .GetProperty("offlineGeoworldAlphaManualAcceptanceQualityGatePassed")
            .GetBoolean());
        ...
        AssertProofPassed(proofs, "goal106.unity_script_inventory");
        AssertProofPassed(proofs, "goal106.editor_window_inventory");
        AssertProofPassed(proofs, "goal106.simulated_save_load_replay");
        AssertProofPassed(proofs, "goal106.negative");
        AssertProofPassed(proofs, "goal106.alpha_runtime_bootstrap_unchanged");
        AssertProofPassed(proofs, "goal106.checkpoint_resume");
        AssertProofPassed(proofs, "goal106.final_hash");
        AssertProofPassed(proofs, "goal106.quality_gate");
        AssertProofPassed(proofs, "goal107.unity_script_inventory");
        AssertProofPassed(proofs, "goal107.editor_window_inventory");
        AssertProofPassed(proofs, "goal107.replay_acceptance");
        AssertProofPassed(proofs, "goal107.negative");
        AssertProofPassed(proofs, "goal107.checkpoint_resume");
        AssertProofPassed(proofs, "goal107.completion_transitions");
        AssertProofPassed(proofs, "goal107.alpha_quality_consolidation");
        AssertProofPassed(proofs, "goal107.alpha_runtime_bootstrap_unchanged");
        AssertProofPassed(proofs, "goal107.quality_gate");
        AssertProofPassed(proofs, "goal108.alpha_slice.unity_script_inventory");
        AssertProofPassed(proofs, "goal108.alpha_slice.editor_window_inventory");
        AssertProofPassed(proofs, "goal108.alpha_slice.full_slice_simulated_proof");
        ...
        AssertProofPassed(proofs, "goal110.manual_acceptance.manifest");
        AssertProofPassed(proofs, "goal110.manual_acceptance.checklist");
        AssertProofPassed(proofs, "goal110.manual_acceptance.result_template");
        AssertProofPassed(proofs, "goal110.manual_acceptance.dashboard");
        AssertProofPassed(proofs, "goal110.manual_acceptance.unity_scripts");
        AssertProofPassed(proofs, "goal110.manual_acceptance.editor_window");
        AssertProofPassed(proofs, "goal110.manual_acceptance.simulated_proof");
        AssertProofPassed(proofs, "goal110.manual_acceptance.negative_proof");
        AssertProofPassed(proofs, "goal110.manual_acceptance.workspace_binding");
        AssertProofPassed(proofs, "goal110.manual_acceptance.quality_gate");
        ...
        Assert.True(
            binding.RootElement.GetProperty("pageBindDisplaysOfflineGeoworldAlphaManualAcceptance").GetBoolean());
        ...
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldSessionReplayGroupPresent").GetBoolean());
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldSessionReplayStepCount").GetInt32() >= 6);
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldSessionStateDeltaCount").GetInt32() >= 6);
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldSessionCheckpointStepIndex").GetInt32() >= 3);
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldSessionAcceptanceChecklistStepCount").GetInt32() > 0);
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldSessionUnityScriptsReady").GetBoolean());
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldSessionEditorWindowReady").GetBoolean());
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldSessionSimulatedReplayProofPassed").GetBoolean());
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldSessionNegativeProofPassed").GetBoolean());
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldSessionAlphaRuntimeBootstrapUnchanged").GetBoolean());
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldSessionQualityGatePassed").GetBoolean());
        Assert.True(quality.RootElement.GetProperty("goal106FilesDiscoveredByRelativePaths").GetBoolean());
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldObjectiveAcceptanceGroupPresent").GetBoolean());
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldObjectiveCount").GetInt32() >= 6);
        Assert.Equal(
            quality.RootElement.GetProperty("offlineGeoworldObjectiveCount").GetInt32(),
            quality.RootElement.GetProperty("offlineGeoworldObjectiveCompletedCount").GetInt32());
        Assert.Equal(6, quality.RootElement.GetProperty("offlineGeoworldObjectivePayloadFileCount").GetInt32());
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldObjectiveReplayAcceptanceProofPassed").GetBoolean());
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldObjectiveAlphaQualityConsolidationPassed").GetBoolean());
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldObjectiveQualityGatePassed").GetBoolean());
        Assert.True(quality.RootElement.GetProperty("goal107FilesDiscoveredByRelativePaths").GetBoolean());
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldAlphaSliceGroupPresent").GetBoolean());
        Assert.Equal(7, quality.RootElement.GetProperty("offlineGeoworldAlphaSliceComponentCount").GetInt32());
        Assert.Equal(7, quality.RootElement.GetProperty("offlineGeoworldAlphaSliceReadyComponentCount").GetInt32());
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldAlphaSliceObjectiveCount").GetInt32() >= 5);
        Assert.Equal(
            quality.RootElement.GetProperty("offlineGeoworldAlphaSliceObjectiveCount").GetInt32(),
            quality.RootElement.GetProperty("offlineGeoworldAlphaSliceCompletedObjectiveCount").GetInt32());
        Assert.Equal("completed", quality.RootElement.GetProperty("offlineGeoworldAlphaSliceFinalStatus").GetString());
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldAlphaSliceUnityToolReady").GetBoolean());
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldAlphaSliceAcceptanceRunbookReady").GetBoolean());
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldAlphaSliceFinalProofPassed").GetBoolean());
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldAlphaSliceNegativeProofPassed").GetBoolean());
        ...
        Assert.True(quality.RootElement.GetProperty("offlineGeoworldAlphaManualAcceptanceGroupPresent").GetBoolean());
        Assert.Equal(
            12,
            quality.RootElement
                .GetProperty("offlineGeoworldAlphaManualAcceptanceChecklistStepCount")
        ...
        Assert.Equal(
            ...
            quality.RootElement
                .GetProperty("offlineGeoworldAlphaManualAcceptanceExportFileCount")
                .GetInt32());
        Assert.True(quality.RootElement.GetProperty(
            "offlineGeoworldAlphaManualAcceptanceAutomatedGatePassed").GetBoolean());
        Assert.True(quality.RootElement.GetProperty(
            "offlineGeoworldAlphaManualAcceptanceManualPending").GetBoolean());
        Assert.True(quality.RootElement.GetProperty(
            "offlineGeoworldAlphaManualAcceptanceUnityRunnerReady").GetBoolean());
        Assert.True(quality.RootElement.GetProperty(
            "offlineGeoworldAlphaManualAcceptanceSimulatedProofPassed").GetBoolean());
        Assert.True(quality.RootElement.GetProperty(
            "offlineGeoworldAlphaManualAcceptanceNegativeProofPassed").GetBoolean());
        Assert.True(quality.RootElement.GetProperty(
            "offlineGeoworldAlphaManualAcceptanceAlphaRuntimeBootstrapUnchanged").GetBoolean());
        Assert.True(quality.RootElement.GetProperty(
            "offlineGeoworldAlphaManualAcceptanceQualityGatePassed").GetBoolean());
        Assert.True(quality.RootElement.GetProperty("goal110FilesDiscoveredByRelativePaths").GetBoolean());
        ...
        var goal106ArtifactRoot = Path.Combine(
            repoRoot,
            ".llmgc",
            "procedural",
            "goal-106-offline-geoworld-session-persistence-replay");
        var goal106StreamingRoot = Path.Combine(
            repoRoot,
            "unity",
            "LLMGameCreatorAlpha",
            "Assets",
        ...
        var goal107ArtifactRoot = Path.Combine(
            repoRoot,
            ".llmgc",
            "procedural",
            "goal-107-offline-geoworld-objective-acceptance-run");
        var goal107StreamingRoot = Path.Combine(
            repoRoot,
            "unity",
            "LLMGameCreatorAlpha",
            "Assets",
        ...
        var goal110ArtifactRoot = Path.Combine(
            repoRoot,
            ".llmgc",
            "procedural",
            "goal-110-offline-geoworld-alpha-manual-acceptance-gate");
        var goal110ExportRoot = Path.Combine(
            repoRoot,
            ".llmgc",
            "exports",
            "goal-110-offline-geoworld-alpha-acceptance");
        var goal110StreamingRoot = Path.Combine(
            repoRoot,
            "unity",
            "LLMGameCreatorAlpha",
            "Assets",
        ...
        var mediaFiles = new[]
            {
                ...
                goal106ArtifactRoot,
                goal106StreamingRoot,
                goal107ArtifactRoot,
                goal107StreamingRoot,
                goal110ArtifactRoot,
                goal110ExportRoot,
                goal110StreamingRoot
            }
        ...
        Assert.Contains("offlineGeoworldSessionReplayStepCount:", report);
        Assert.Contains("offlineGeoworldSessionStateDeltaCount:", report);
        Assert.Contains("offlineGeoworldSessionCheckpointStepIndex:", report);
        Assert.Contains("offlineGeoworldSessionUnityScriptsReady: true", report);
        Assert.Contains("offlineGeoworldSessionEditorWindowReady: true", report);
        Assert.Contains("offlineGeoworldSessionSimulatedReplayProofPassed: true", report);
        Assert.Contains("offlineGeoworldSessionNegativeProofPassed: true", report);
        Assert.Contains("offlineGeoworldSessionAlphaRuntimeBootstrapUnchanged: true", report);
        Assert.Contains("offlineGeoworldSessionQualityGatePassed: true", report);
        Assert.Contains("goal106FilesDiscoveredByRelativePaths: true", report);
        Assert.Contains("offlineGeoworldObjectiveCount:", report);
        Assert.Contains("offlineGeoworldObjectiveReplayAcceptanceProofPassed: true", report);
        Assert.Contains("offlineGeoworldObjectiveAlphaQualityConsolidationPassed: true", report);
        Assert.Contains("offlineGeoworldObjectiveQualityGatePassed: true", report);
        Assert.Contains("goal107FilesDiscoveredByRelativePaths: true", report);
        Assert.Contains("offlineGeoworldAlphaSliceComponentCount: 7", report);
        Assert.Contains("offlineGeoworldAlphaSliceUnityToolReady: true", report);
        Assert.Contains("offlineGeoworldAlphaSliceFinalProofPassed: true", report);
        Assert.Contains("offlineGeoworldAlphaSliceQualityGatePassed: true", report);
        Assert.Contains("goal108FilesDiscoveredByRelativePaths: true", report);
        Assert.Contains("offlineGeoworldAlphaManualAcceptanceChecklistStepCount: 12", report);
        Assert.Contains("offlineGeoworldAlphaManualAcceptanceManualPending: true", report);
        Assert.Contains("offlineGeoworldAlphaManualAcceptanceUnityRunnerReady: true", report);
        Assert.Contains("offlineGeoworldAlphaManualAcceptanceQualityGatePassed: true", report);
        Assert.Contains("goal110FilesDiscoveredByRelativePaths: true", report);
