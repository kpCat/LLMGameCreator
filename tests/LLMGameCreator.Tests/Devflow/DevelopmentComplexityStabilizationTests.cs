...
public sealed class DevelopmentComplexityStabilizationTests
{
    ...
    public void StateDocsRecordGoal022AcceptedBeforeGoal023()
    {
        ...
        Assert.Equal("package_assembly_world_entities_expansion_verification", state.RootElement.GetProperty("gate_status").GetString());
        Assert.Equal("goal_025_package_assembly_expansion_1_world_and_entities", state.RootElement.GetProperty("last_completed_product_slice_id").GetString());
        ...
        Assert.Contains("package_assembly_world_entities_expansion_verification", contextIndex);
    }
