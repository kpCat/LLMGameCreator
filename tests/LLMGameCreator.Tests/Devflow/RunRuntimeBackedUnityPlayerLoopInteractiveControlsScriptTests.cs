...
public sealed class RunRuntimeBackedUnityPlayerLoopInteractiveControlsScriptTests
{
    [Fact]
    public void RunRuntimeBackedUnityPlayerLoopInteractiveControlsScriptExposesGoal139Contract()
    {
        ...
        var script = File.ReadAllText(Path.Combine(
            ...
            ".devflow",
            "scripts",
            "run-runtime-backed-unity-player-loop-interactive-controls.ps1"));
        var cmd = File.ReadAllText(Path.Combine(
            root,
            ".devflow",
            "scripts",
            "run-runtime-backed-unity-player-loop-interactive-controls.cmd"));

        Assert.Contains("[string]$StepperModelPath", script, StringComparison.Ordinal);
        Assert.Contains("[string]$StepperResultPath", script, StringComparison.Ordinal);
        ...
        Assert.Contains("OutputRoot must stay under the Goal139 output root", script, StringComparison.Ordinal);
        Assert.Contains("RuntimeBackedUnityPlayerLoopInteractiveControlsScriptProof", script, StringComparison.Ordinal);
        Assert.Contains(
        ...
        Assert.Contains(
            ...
            StringComparison.Ordinal);
        Assert.Contains(
            "RunBatchmodeRuntimeBackedUnityPlayerLoopInteractiveControlsSmoke",
            script,
            StringComparison.Ordinal);
        Assert.Contains(
        ...
        Assert.Contains(
            ...
            script,
            StringComparison.Ordinal);
        Assert.Contains("unity-player-loop-interactive-controls-smoke.json", script, StringComparison.Ordinal);
        Assert.Contains("CanonicalRuntimeUnityPlayerLoopInteractiveControlsWindow.cs", script, StringComparison.Ordinal);
        Assert.Contains("LLMGC_GOAL139_UNITY_SMOKE_PATH", script, StringComparison.Ordinal);
        ...
        Assert.Contains("run-runtime-backed-unity-player-loop-interactive-controls.ps1", cmd, StringComparison.Ordinal);
